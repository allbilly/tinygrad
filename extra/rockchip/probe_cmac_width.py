...
def image(channels:int):
  out = RKTensorRef(RKArg(RKBufferKind.ARG,0),RKLayout((1,channels),(1,64),(128,2),dtypes.half,
    padding=((0,0),(0,64-channels))))
  lhs = RKTensorRef(RKArg(RKBufferKind.ARG,1),RKLayout((1,32),(1,32),(64,2),dtypes.half))
  rhs = RKTensorRef(RKArg(RKBufferKind.ARG,2),RKLayout((channels,32),(32,32),(64,2),dtypes.half,
    padding=((0,32-channels),(0,0)),kind=RKLayoutKind.CMAC_WEIGHT))
  return emit_contract(RKContract(out,lhs,rhs,0))

def main() -> None:
  dev = RockchipDevice("ROCKCHIP")
  lhs = np.linspace(-3,3,32,dtype=np.float16)
  for channels in (16,20,24,28,32):
    weight = np.eye(32,dtype=np.float16)
    packed = weight.reshape(2,16,1,32).transpose(0,2,1,3).ravel()
    out, lhs_buf, rhs = dev._gpu_alloc(128), dev._gpu_alloc(lhs.nbytes), dev._gpu_alloc(packed.nbytes)
    try:
      ...
      ctypes.memset(int(out.va_addr),0,128)
      ...
      physical = np.frombuffer(ctypes.string_at(int(out.va_addr),128),dtype=np.float16).copy()
