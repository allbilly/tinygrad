...
def _try_round_subtasks(sink:UOp) -> tuple[RKSubTask, ...]|None:
  ...
  fp32_inputs = (source_slot,) if fp32_in else ()
  source_arg, negative_one = (source_slot,0), _float_arg(-1.0)
  negative = graph.op(source_arg, negative_one, Ops.MUL, repeat=False, fp32_inputs=fp32_inputs)
  magnitude = graph.op(source_arg, negative, Ops.MAX, repeat=False, fp32_inputs=fp32_inputs)
  rounded = graph.alloc()
  if not graph.emit_named_lut("rk_roundoff", magnitude[0], rounded): return None
  zero = (_ZERO_SLOT, 0)
  negative_diff = graph.op(zero, source_arg, Ops.SUB, repeat=False, fp32_inputs=fp32_inputs)
  negative_mask = graph.op(negative_diff, negative_diff, Ops.MAX, repeat=False, compare=True)
  positive_diff = graph.op(source_arg, zero, Ops.SUB, repeat=False, fp32_inputs=fp32_inputs)
  positive_mask = graph.op(positive_diff, positive_diff, Ops.MAX, repeat=False, compare=True)
  sign = graph.op(positive_mask, negative_mask, Ops.SUB)
  graph.op((rounded,0), sign, Ops.MUL, out_slot=graph.out, fp32_output=fp32_in)
...
def _try_sign_subtasks(sink:UOp) -> tuple[RKSubTask, ...]|None:
  ...
  source = next(u for u in store.src[1].toposort() if u.op is Ops.INDEX and u.src[0].buf_uop.arg.slot == input_slot)
  graph = _TaskGraph(sink, store, source)
  zero = (_ZERO_SLOT, 0)
  fp32_in = store.src[0].src[0].dtype is dtypes.float
  fp32_inputs = (input_slot,) if fp32_in else ()
  source_arg = (input_slot,0)
  negative_diff = graph.op(zero, source_arg, Ops.SUB, repeat=False, fp32_inputs=fp32_inputs)
  negative = graph.op(negative_diff, negative_diff, Ops.MAX, repeat=False, compare=True)
  positive_diff = graph.op(source_arg, zero, Ops.SUB, repeat=False, fp32_inputs=fp32_inputs)
  positive = graph.op(positive_diff, positive_diff, Ops.MAX, repeat=False, compare=True)
  graph.op(positive, negative, Ops.SUB, out_slot=graph.out, fp32_output=fp32_in)
  return tuple(graph.tasks)
...
def _try_inf_div_subtasks(sink:UOp) -> tuple[RKSubTask, ...]|None:
  ...
  graph = _TaskGraph(sink, store, denominator)
  denominator_arg = (denominator.src[0].buf_uop.arg.slot, 0)
  numerator_arg = _float_arg(float(val.src[0].arg))
  fp32_in = store.src[0].src[0].dtype is dtypes.float
  fp32_inputs = (denominator_arg[0],) if fp32_in else ()
  zero = (_ZERO_SLOT, 0)
  base = graph.op(numerator_arg, denominator_arg, Ops.FDIV, repeat=False, fp32_inputs=fp32_inputs)
  negative_diff = graph.op(zero, denominator_arg, Ops.SUB, repeat=False, fp32_inputs=fp32_inputs)
  negative = graph.op(negative_diff, negative_diff, Ops.MAX, repeat=False, compare=True)
  positive_diff = graph.op(denominator_arg, zero, Ops.SUB, repeat=False, fp32_inputs=fp32_inputs)
  positive = graph.op(positive_diff, positive_diff, Ops.MAX, repeat=False, compare=True)
  sign = graph.op(positive, negative, Ops.SUB)
  graph.op(base, sign, Ops.MUL, out_slot=graph.out, fp32_output=fp32_in)
  return tuple(graph.tasks)
...
def _try_hardsigmoid_subtasks(sink:UOp) -> tuple[RKSubTask, ...]|None:
  ...
  source_index = next(u for u in store.src[1].toposort() if u.op is Ops.INDEX and u.src[0].buf_uop.arg.slot == input_slot)
  graph = _TaskGraph(sink, store, source_index)
  source, zero, negative_one = (input_slot, 0), (_ZERO_SLOT, 0), _float_arg(-1.0)
  scaled = graph.op(source, _float_arg(alpha), Ops.MUL, repeat=False)
  shifted = graph.op(scaled, _float_arg(beta), Ops.ADD, repeat=False)
  positive = graph.op(shifted, zero, Ops.MAX, repeat=False)
  negative = graph.op(positive, negative_one, Ops.MUL, repeat=False)
  clamped = graph.op(negative, _float_arg(-1.0), Ops.MAX, repeat=False)
  graph.op(clamped, negative_one, Ops.MUL, out_slot=graph.out, repeat=False)
  return tuple(graph.tasks)
...
def _try_quick_gelu_saturation_subtasks(sink:UOp) -> tuple[RKSubTask, ...]|None:
  ...
  if not graph.extend(_try_elementwise_subtasks(graph.stage_sink(store.src[1], base_slot))): return None
  source_arg, one = (source.src[0].buf_uop.arg.slot, 0), _float_arg(1.0)
  low_mask = graph.positive_mask(_float_arg(-10.0), source_arg)
  high_mask = graph.positive_mask(source_arg, _float_arg(5.0))
  outside = graph.op(high_mask, low_mask, Ops.MAX)
  inside = graph.op(one, outside, Ops.SUB)
  base_selected = graph.op((base_slot,0), inside, Ops.MUL)
  high_selected = graph.op(source_arg, high_mask, Ops.MUL)
  graph.op(base_selected, high_selected, Ops.ADD, out_slot=graph.out)
  return tuple(graph.tasks)
...
def _try_pow8_lut_subtasks(sink:UOp) -> tuple[RKSubTask, ...]|None:
  ...
  square = graph.op(source_arg, source_arg, Ops.MUL, repeat=False)
  fourth = graph.op(square, square, Ops.MUL, repeat=False)
  repeated = graph.op(fourth, fourth, Ops.MUL, repeat=False)
  ...
  bands = (band0, band1, band2, above_two)
  multiplier = graph.weighted_sum(bands, (4.0, 2.0, 1.0, 0.5))
  factor = graph.weighted_sum(bands, (2.0**-16, 2.0**-8, 1.0, 256.0))
  normalized = graph.op(absolute, multiplier, Ops.MUL)

  base_value = UOp(Ops.CUSTOM, dtypes.half, (graph.temp_index(normalized[0]),), arg="rk_pow8")
  base = graph.custom_lut(base_value, (normalized[0],), _LUT_POW8)
  high_value = UOp(Ops.CUSTOM, dtypes.half, (graph.temp_index(normalized[0]),), arg="rk_pow8_high")
  high = graph.custom_lut(high_value, (normalized[0],), _LUT_POW8_HIGH)

  high_mask = graph.positive_mask(normalized, _float_arg(float(np.float16(math.sqrt(2.0)))))
  high_scaled = graph.op(high, _float_arg(256.0), Ops.MUL)
  low_mask = graph.op(one, high_mask, Ops.SUB)
  low_selected = graph.op(base, low_mask, Ops.MUL)
  high_selected = graph.op(high_scaled, high_mask, Ops.MUL)
  corrected = graph.op(low_selected, high_selected, Ops.ADD)
  scaled = graph.op(corrected, factor, Ops.MUL)
  ...
  if (debug_stage := getenv("ROCKCHIP_DEBUG_POW8_STAGE")):
    debug_slots = {1:normalized, 2:base, 3:high, 4:high_scaled, 5:corrected, 6:factor, 7:bounded}
    if debug_stage in debug_slots:
      graph.op(debug_slots[debug_stage], (_ZERO_SLOT, 0), Ops.ADD, out_slot=out)
      return tuple(tasks)

  above_quarter = graph.positive_mask(absolute, _float_arg(0.25))
  below_four = graph.positive_mask(_float_arg(4.0), absolute)
  valid = graph.op(above_quarter, below_four, Ops.MUL)
  selected = graph.op(bounded, valid, Ops.MUL)
  inverse = graph.op(one, valid, Ops.SUB)
  fallback = graph.op(repeated, inverse, Ops.MUL)
  graph.op(selected, fallback, Ops.ADD, out_slot=out)
  return tuple(tasks)
...
def _try_pow55_lut_subtasks(sink:UOp) -> tuple[RKSubTask, ...]|None:
  ...
  base_node = reciprocals[0] if reciprocals else source

  def exponent(u:UOp) -> float|None:
    u = _unwrap(u)
    if u is base_node: return 1.0
    if u.op is Ops.SQRT and len(u.src) == 1 and _unwrap(u.src[0]) is base_node: return 0.5
    if u.op is not Ops.MUL or len(u.src) != 2: return None
    lhs, rhs = exponent(u.src[0]), exponent(u.src[1])
    return None if lhs is None or rhs is None else lhs+rhs
  ...
  base_arg = graph.op(one, source_arg, Ops.FDIV) if reciprocals else source_arg
  negative = graph.op(base_arg, _float_arg(-1.0), Ops.MUL)
  absolute = graph.op(base_arg, negative, Ops.MAX)
  above_half = graph.positive_mask(absolute, _float_arg(0.5))
  above_one = graph.positive_mask(absolute, one)
  above_two = graph.positive_mask(absolute, _float_arg(2.0))
  band0 = graph.op(one, above_half, Ops.SUB)
  ...
  bands = (band0, band1, band2, above_two)
  multiplier = graph.weighted_sum(bands, (4.0, 2.0, 1.0, 0.5))
  factor = graph.weighted_sum(bands, (2.0**-11, 2.0**-5.5, 1.0, 2.0**5.5))
  normalized = graph.op(absolute, multiplier, Ops.MUL)

  # Negative-5.5 fine-grid WIP uses a shifted high_input; positive 5.5 keeps
  # the unshifted normalized coordinate.
  low_value = UOp(Ops.CUSTOM, dtypes.half, (graph.temp_index(normalized[0]),), arg="rk_pow55")
  low = graph.custom_lut(low_value, (normalized[0],), _LUT_POW55)
  high_value = UOp(Ops.CUSTOM, dtypes.half, (graph.temp_index(normalized[0]),), arg="rk_pow55_high")
  high = graph.custom_lut(high_value, (normalized[0],), _LUT_POW55_HIGH)
  ...
  high_mask = graph.positive_mask(normalized, _float_arg(float(split)))
  high_scaled = graph.op(high, _float_arg(2.0**5.5), Ops.MUL)
  low_mask = graph.op(one, high_mask, Ops.SUB)
  ...
  if (debug_stage := getenv("ROCKCHIP_DEBUG_POW55_STAGE")):
    debug_slots = {1:normalized, 2:low, 3:high, 4:high_scaled, 5:normalized_power, 6:factor, 7:bounded}
    if debug_stage in debug_slots:
      graph.op(debug_slots[debug_stage], (_ZERO_SLOT,0), Ops.ADD, out_slot=out)
      return tuple(tasks)
  ...
  combined = graph.op(selected, fallback_selected, Ops.ADD)
  negative_mask = graph.positive_mask((_ZERO_SLOT,0), base_arg)
  negative_inf = graph.positive_mask(_float_arg(-65472.0), base_arg)
  invalid_negative = graph.op(negative_mask, negative_inf, Ops.SUB)
  invalid_denom = graph.op(one, invalid_negative, Ops.SUB)
  invalid_factor = graph.op(invalid_denom, invalid_denom, Ops.FDIV)
  graph.op(combined, invalid_factor, Ops.MUL, out_slot=out)
  return tuple(tasks)
...
class _TaskGraph:
  """Build reset-separated DPU task graphs while owning scratch-slot visibility."""
  ...
  def dependent(self, out_slot:int, lhs:tuple[int,int], rhs:tuple[int,int], op:Ops, **kwargs) -> tuple[int,int]:
    ...
    self.tasks.append(_emit_where_stage(self.total, self.alloc(), lhs, rhs, op, **scratch_kwargs))
    self.tasks.append(_emit_where_stage(self.total, out_slot, lhs, rhs, op, **kwargs))
    return out_slot, 0

  def op(self, lhs:tuple[int,int], rhs:tuple[int,int], op:Ops, out_slot:int|None=None,
         repeat:bool=True, **kwargs) -> tuple[int,int]:
    out_slot = self.alloc() if out_slot is None else out_slot
    if repeat: return self.dependent(out_slot, lhs, rhs, op, **kwargs)
    self.tasks.append(_emit_where_stage(self.total, out_slot, lhs, rhs, op, **kwargs))
    return out_slot, 0
  ...
  def extend(self, stage_tasks:tuple[RKSubTask, ...]|None) -> bool:
    ...
    self.tasks.extend(stage_tasks)
    used_slots = [task.task.out_slot for task in stage_tasks] + \
      [reloc.globals_slot for task in stage_tasks for reloc in task.relocs if reloc.globals_slot not in (_CONST_SLOT, _ZERO_SLOT)]
  ...
  def custom_lut(self, val:UOp, in_slots:tuple[int, ...], lut_op:Ops) -> tuple[int,int]:
    out_slot = self.alloc()
    plan = RKPlan("dpu_lut", self.stage_sink(val, out_slot), out_slot, in_slots, lut_op=lut_op)
    cmds, task, relocs = emit_rk(plan)
    self.tasks.append(RKSubTask(cmds, task, relocs))
    return out_slot, 0

  def emit_named_lut(self, name:object, input_slot:int, out_slot:int) -> bool:
    value = UOp(Ops.CUSTOM, dtypes.half, (self.temp_index(input_slot),), arg=name)
    return self.emit_lut(value, out_slot)
...
def _try_cumprod_host_subtasks(sink:UOp) -> tuple[RKSubTask, ...]|None:
  ...
  if store is None or store.src[0].dtype is not dtypes.half or len(reductions) != 1: return None
  reduce = reductions[0]
  if reduce.dtype is not dtypes.half or reduce.arg[0] is not Ops.MUL or len(reduce.src) != 2 or \
     reduce.src[1].src[0].op is not Ops.CONST: return None
  body = reduce.src[0]
  body_nodes, value_nodes = body.toposort(), store.src[1].toposort()
  indexes = [u for u in body_nodes if u.op is Ops.INDEX and u.dtype is dtypes.half and u.src[0].op is Ops.PARAM]
  if body.op is not Ops.WHERE or len(indexes) != 1: return None
  source = indexes[0]
  window = int(reduce.src[1].src[0].arg)
  loops = [u for u in sink.toposort() if u.op is Ops.RANGE and getattr(u.arg[-1], "name", "") == "LOOP"]
  if any(u.src[0].op is not Ops.CONST for u in loops): return None
  ...
  allowed = {Ops.REDUCE, Ops.WHERE, Ops.AND, Ops.CMPLT, Ops.CMPNE, Ops.ADD, Ops.MUL, Ops.INDEX, Ops.PARAM, Ops.RANGE, Ops.CONST}
  ...
  padded_blocks = store.src[1] is reduce and total == 1024 and input_total == 1022 and window == 256 and \
                  sorted(loop_extents) == [4, 256] and sum(u.op is Ops.WHERE for u in body_nodes) == 5 and \
                  sum(u.op is Ops.CMPLT for u in body_nodes) == 2 and sum(u.op is Ops.CMPNE for u in body_nodes) == 2 and \
                  sum(u.op is Ops.AND for u in body_nodes) == 1
  block_prefix = store.src[1].op is Ops.WHERE and reduce in value_nodes and total == 4 and input_total == 1024 and \
                 window == 4 and loop_extents == [4] and sum(u.op is Ops.WHERE for u in value_nodes) == 6 and \
                 sum(u.op is Ops.CMPLT for u in value_nodes) == 2 and sum(u.op is Ops.CMPNE for u in value_nodes) == 2 and \
                 sum(u.op is Ops.AND for u in value_nodes) == 1
  ...
  if store.src[1] is not reduce or input_total != total or sum(u.op is Ops.WHERE for u in body_nodes) != 3 or \
     sum(u.op is Ops.CMPLT for u in body_nodes) != 1 or sum(u.op is Ops.CMPNE for u in body_nodes) != 1: return None
  ...
  if condition.src[0].op is not Ops.ADD or condition.src[1].op is not Ops.CONST or int(condition.src[1].arg) != window-1 or \
     body.src[0] is not condition or body.src[1].op is not Ops.CONST or float(body.src[1].arg) != 1.0: return None
