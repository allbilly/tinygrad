...
def _try_sinh_cosh_subtasks(sink:UOp) -> tuple[RKSubTask, ...]|None:
  ...
  graph = _TaskGraph(sink, store, source)
  total, out, tasks, alloc = graph.total, graph.out, graph.tasks, graph.alloc
  source_arg, zero, one = (source.src[0].buf_uop.arg.slot, 0), (_ZERO_SLOT, 0), _float_arg(1)
  ...
  tasks.extend((_emit_where_stage(total, low, source_arg, _float_arg(-2), Ops.MAX),
                ...
                _emit_where_stage(total, bounded, zero, (neg_clamped, 0), Ops.SUB)))
  lut_slot = alloc()
  if not graph.emit_named_lut("rk_cosh" if is_cosh else "rk_sinh", bounded, lut_slot): return None
  local_slot = -1
  if not is_cosh:
    local_slot = alloc()
    if not graph.emit_named_lut("rk_sinh_local", bounded, local_slot): return None
  ...
  tasks.extend((_emit_where_stage(total, large_diff, (abs_source, 0), _float_arg(10), Ops.SUB),
                ...
                _emit_where_stage(total, out, (selected, 0), (denom, 0), Ops.FDIV)))
  return tuple(tasks)
