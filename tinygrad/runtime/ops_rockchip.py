...
class RockchipProgram(Program['RockchipDevice']):
  ...
  def _submit_bodies(self, bodies:typing.Iterable[tuple[int, ...]], standalone:bool=False, cmac:bool=False) -> None:
    ...
    for i,(body,size) in enumerate(zip(bodies,sizes)):
      ...
      words[offsets[i]:offsets[i]+size+len(tail)]=array.array("Q",body+tail); descs[i]=rk.struct_rknpu_task(0,0 if cmac else 4,0xd if cmac else 0x18,0x300,0x1ffff,0,size if cmac else size+len(tail),0,base_dma+offsets[i]*8)  # noqa: E501,E702
  ...
  def _run_ew_ops(self, address, ops:tuple[RKEWOp, ...], rearm:bool=False) -> None:
    ...
    for begin,end in zip(cuts,cuts[1:]):
      ...
      for group in map(tuple,itertools.batched(section,limit)):
        ...
        precisions=tuple(_EW_MODE_INFO[op.mode][0] for op in group)
  ...
  def __call__(self, *bufs:HCQBuffer, global_size=(1,1,1), local_size=(1,1,1), vals=(), wait=False, **kwargs):
    with self.dev._lock:
      ...
      if ew_ops and self.dev._native_int16 and ew_ops[0].mode not in (RKEWMode.INT16,RKEWMode.INT16_TO_INT32,RKEWMode.HALF_TO_INT16): self.dev.reset_npu()  # noqa: E501
