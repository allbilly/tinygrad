...
_arch_map = {"gfx8": "gcn3", "gfx9": "cdna", "gfx10": "rdna3", "gfx11": "rdna3", "gfx12": "rdna4"}
def assemble_linear(prg:UOp, lin:UOp, arch:str) -> bytes:
  ...
  for inst in insts:
    # Hand-written ISA encoders can report register counts without depending on
    # the PDF-generated instruction classes used by RDNA/CDNA.
    if (counts := getattr(inst, "register_counts", None)) is not None:
      max_vgpr, max_sgpr, max_accvgpr = map(max, (max_vgpr, max_sgpr, max_accvgpr), counts)
      continue
  ...
  padding_inst = (s_nop_cdna(0) if is_gcn3 or is_cdna else s_code_end()).to_bytes()
  padding_bytes = (hsa.AMD_ISA_ALIGN_BYTES - len(code_bytes) % hsa.AMD_ISA_ALIGN_BYTES) % hsa.AMD_ISA_ALIGN_BYTES
  assert padding_bytes % len(padding_inst) == 0
  text = code_bytes + padding_inst * (padding_bytes // len(padding_inst))
  ...
  desc.compute_pgm_rsrc1 = (vgpr_granule << amdgpu_kd.COMPUTE_PGM_RSRC1_GRANULATED_WORKITEM_VGPR_COUNT_SHIFT |
                            ...
                            (3 if is_gcn3 else 0) << amdgpu_kd.COMPUTE_PGM_RSRC1_FLOAT_DENORM_MODE_32_SHIFT |
