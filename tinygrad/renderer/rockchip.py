...
def _lower_mapped_reduce(output:RKOutput, uops:list[UOp], plan:RKPlan) -> bool:
  ...
  if rows>1 and (value.arg[0] not in ((Ops.MUL,) if boolean else (Ops.ADD,Ops.MAX,Ops.MUL)) or output_axes is None or lanes>(_MAX_GENERIC_EXPANDED_NODES if boolean else min(_MAX_STATIC_RANGE_ENVS,16*_MAX_GENERIC_UNROLL) if short_math or mapped_cmac or value.arg[0] is Ops.MUL or value.arg[0] is Ops.ADD and product.op in (Ops.EXP2,Ops.LOG2,Ops.SQRT,Ops.SIN) else _MAX_STATIC_RANGE_ENVS if integer or value.arg[0] is Ops.MAX else _MAX_GENERIC_UNROLL)): return False  # noqa: E501
  ...
  replacements={axis:point.alu(Ops.CDIV,point.const_like(stride)).alu(Ops.CMOD,point.const_like(extent)) if stride>1 else point.alu(Ops.CMOD,point.const_like(extent)) for point,dimensions in ((logical,zip(ranges,(math.prod(extents[i+1:]) for i in range(len(extents))),extents)),(output_row,output_axes or ())) for axis,stride,extent in dimensions}  # noqa: E501
...
def _compare_int32_words(op:Ops, lhs:tuple[UOp,...], rhs:tuple[UOp,...]) -> UOp:
