...
def _linear_index(u:UOp, divided:bool=False) -> tuple[int, dict[UOp|tuple[UOp, int, int], int]]|None:
  """Collect scaled affine address terms."""
  # Widening an integer index to weakint keeps its value; retain float-to-integer rounding inside the static evaluator.
  if divided and u.op is Ops.CAST and len(u.src)==1:
    source=u.src[0]
    if u.dtype.scalar() in (dtypes.int,dtypes.uint) or u.dtype.scalar() is dtypes.weakint and dtypes.is_int(source.dtype.scalar()): u=source
  if u.op is Ops.CONST: return int(u.arg), {}
  # A zero period means no modulo.
  ...
  if period and axis.vmin>=0 and axis.vmax<period: return _linear_index(axis,divided)
  ...
  lhs=_linear_index(u.src[0],divided)
  if lhs is None: return None
  rhs=_linear_index(u.src[1],divided)
  if rhs is None or u.op is Ops.MUL and lhs[1] and rhs[1]: return None
  if u.op is Ops.MUL:
  ...
  # Keep first-seen term order so physical schedules do not depend on UOp allocation identities.
...
def _lower_cmac_reduce(output:RKOutput, plan:RKPlan) -> bool:
  ...
  for load in (left,right):
    if load.op is not Ops.LOAD or load.dtype.scalar() is not dtypes.half or not load.src or load.src[0].op is not Ops.INDEX: return False
...
class RKContext:
  ...
  def _compare(self, u:UOp) -> UOp:
    ...
    if integer16 or integer32:
      ...
      components = tuple(self._unpack_words(source) for source in values)
    else:
