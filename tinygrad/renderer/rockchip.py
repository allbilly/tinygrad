...
def _lower_mapped_reduce(output:RKOutput, uops:list[UOp], plan:RKPlan) -> bool:
  ...
  else: replacement=plan.parameter(mapped_dtype,rows,_reduce_mapped_rows(plan,plan.resolve(RKArg(RKBufferKind.ARG,fake.arg.slot)),lanes,_EW_CFG[value.arg[0]],rows,int16=integer,barrier=not direct)).index(out_index).load()  # noqa: E501
