...
def _finish_mapped_add_reduction(mapped:RKImage, out_slot:int, rows:int, groups:int, post_scale:float,
  ...
  ops, arena_slot = list(mapped.ew_ops), value_slot+1
...
class RKContext:
  ...
  def _widen_int16(self, u:UOp, source:RKValue) -> RKValue:
    ...
    self.ew_ops.append(RKEWOp(value.arg, source.arg, zero, self.count, _EW_CFG[Ops.ADD], int16_input=True, int32_output=True))
    return value
