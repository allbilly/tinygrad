...
_NATIVE_GUARD = struct.Struct("<III")
_NATIVE_REPAIR = struct.Struct("<HBBIII")
...
@dataclass(frozen=True)
class RKNativeGuard:
  """A post-effect byte guard; it describes checking, never numeric repair."""
  buffer: RKArg; offset: int; size: int; fill: int = 0
...
def _validate_native_image(image:RKImage) -> None:
  ...
  for index,guard in enumerate(native.guards):
    if type(guard) is not RKNativeGuard: raise ValueError(f"invalid native guard[{index}]")
    _native_arg(guard.buffer, f"guard[{index}] buffer")
    if guard.buffer not in write_set: raise ValueError(f"native guard[{index}] is not a write")
    _native_int(guard.offset, 0, (1 << 32)-1, f"guard[{index}] offset"); _native_int(guard.size, 1, (1 << 32)-1, f"guard[{index}] size")
    _native_int(guard.fill, 0, 255, f"guard[{index}] fill")
  for index,repair in enumerate(native.repairs):
    if type(repair) is not RKNativeRepair: raise ValueError(f"invalid native repair[{index}]")
    _native_enum(repair.kind, RKNativeRepairKind, f"repair[{index}] kind")
...
def _encode_native_image(image:RKImage) -> bytes:
  ...
  out = bytearray(_NATIVE_HEADER.pack(RKIMAGE_MAGIC, RKIMAGE_NATIVE_VERSION, int(image.target), int(native.kind), native.flags,
    ...
    len(native.guards), len(native.repairs), len(native.commands), len(native.tail), len(image.constants)))
  ...
  for guard in native.guards:
    _native_put_arg(out, guard.buffer); out += _NATIVE_GUARD.pack(guard.offset, guard.size, guard.fill)
  for repair in native.repairs:
    out += _NATIVE_REPAIR.pack(int(repair.kind), int(repair.fallback), 0, repair.source_mask, repair.source_value, repair.result_bits)
...
def _decode_native_image(blob:bytes) -> RKImage:
  ...
  for _ in range(nguards):
    ...
    if reserved: raise ValueError("invalid native guard argument flags")
    data,off = _native_take(blob, off, _NATIVE_GUARD.size); offset,size,fill = _NATIVE_GUARD.unpack(data)
    guards.append(RKNativeGuard(RKArg(RKBufferKind(source_kind), index, addend), offset, size, fill))
  repairs = []
...
def _finish_mapped_add_reduction(mapped:RKImage, out_slot:int, rows:int, groups:int, post_scale:float,
  ...
  ops, arena_slot = list(mapped.ew_ops), value_slot+1; stride, _, arena, active = _reduction_arena(rows, groups, arena_slot)
...
def _lower_mapped_add_loop_reduction(output:RKOutput, uops:list[UOp]) -> RKImage|None:
  ...
  store, out, rows, out_index, root = output; nodes, out_ranges = list(root.toposort()), _index_ranges(out_index)
  reduce_ranges = [u for u in nodes if u.op is Ops.RANGE and u not in out_ranges]
...
def _lower_vectorized_unrolled_add_reduction(output:RKOutput, uops:list[UOp]) -> RKImage|None:
  ...
  template = terms[0]; loaded_indices = {u.src[0] for u in template.toposort() if u.op is Ops.LOAD and u.src and u.src[0].op is Ops.INDEX}
  ...
  substitutions:dict[UOp, UOp] = {}; static_fallbacks:list[tuple[int, UOp, tuple[int, ...]]] = []
...
def _lower_vectorized_mul_add_reduction(output:RKOutput, uops:list[UOp]) -> RKImage|None:
  ...
  store, out, rows, out_index, root = output; summed, post_scale = root, 1.0
  ...
  terms, post_scale = scaled; groups, lanes, chunk_lanes = len(terms), rows*len(terms), _MAX_EW_ELEMS_FP16*(round_up(2, 64)//2)
