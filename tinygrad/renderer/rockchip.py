...
RKIMAGE_MAGIC, RKIMAGE_VERSION = b"RKIM", 27
...
class RKEWOp:
  ...
  int32_output: bool = False; int32_input: bool = False; bool_output: bool = False; int16_output: bool = False
...
def encode_image(image:RKImage) -> bytes:
  ...
  for op in image.ew_ops:
    ...
    if op.int16_output and (op.int32_output or op.int32_input): raise ValueError("conflicting integer conversion")
    op_flags = (int(op.submit_barrier) | int(op.compare)<<1 | int(op.stateful)<<2 | int(op.int32_output)<<3 |
                int(op.int32_input)<<4 | int(op.bool_output)<<5 | int(op.int16_output)<<6)
...
def decode_image(blob:bytes) -> RKImage:
  ...
  for _ in range(nop):
    ...
    if op_flags & ~0x7f or op_flags & 0x18 == 0x18 or op_flags & 0x20 and not op_flags & 0x08 or \
       op_flags & 0x40 and op_flags & 0x18:
    ...
    ew_ops.append(RKEWOp(RKArg(RKBufferKind(dk), di, da), RKArg(RKBufferKind(lk), li, la),
                         ...
                         bool(op_flags & 32), bool(op_flags & 64)))
  fill = None
  if flags & 1:
...
def _emit_stateful_stage(dst:RKArg, lhs:RKArg, rhs:RKArg, count:int, ew_cfg:int, compare:bool=False,
                         int32_output:bool=False, int32_input:bool=False, int16_output:bool=False) -> RKStage:
  ...
  regs:tuple[tuple[int, int, int], ...] = ((_DPU,rk.REG_DPU_S_POINTER,0xe),
    (_DPU,rk.REG_DPU_FEATURE_MODE_CFG,(15<<5)|(2<<1)|1),
    (_DPU,rk.REG_DPU_DATA_FORMAT,_DPU_DATA_FORMAT_INT32_OUT if int32_output else _DPU_DATA_FORMAT_INT16_OUT if int16_output else
...
def emit_ew_stage(dst:RKArg, lhs:RKArg, rhs:RKArg, count:int, ew_cfg:int, compare:bool=False,
                  stateful:bool=False, int32_output:bool=False, int32_input:bool=False, int16_output:bool=False) -> RKStage:
  ...
  if compare or stateful or int32_output or int32_input or int16_output:
    return _emit_stateful_stage(dst, lhs, rhs, count, ew_cfg, compare, int32_output, int32_input, int16_output)
...
def _positive_half_load(predicate:UOp) -> UOp|None:
  ...
  zero, load = predicate.src
  if (zero.op is not Ops.CONST or zero.dtype.scalar() is not dtypes.half or float(zero.arg) != 0.0 or
...
def _lower_unrolled_fp16_prefix_count(output:RKOutput) -> RKImage|None:
  """Lower fixed masked-select's FP16 positive/nonzero prefix count to exact INT32 lanes."""
  ...
  parsed:dict[UOp, tuple[str, UOp, UOp]] = {}
  for term in terms:
    matches = [("positive", u, load) for u in term.toposort() if (load:=_positive_half_load(u)) is not None]
    matches += [("nonzero", u, load) for u in term.toposort() if (load:=_nonzero_load(u)) is not None]
    loads = [u for u in term.toposort() if u.op is Ops.LOAD]
    if len(matches) != 1 or loads != [matches[0][2]]: return None
    parsed[term] = matches[0]
  modes = {mode for mode,_,_ in parsed.values()}
  if len(modes) != 1: return None
  mode = modes.pop()
  source = _root_param(next(iter(parsed.values()))[2].src[0])
  if source is None or source.src[0].op is not Ops.CONST: return None
  source_count = int(source.src[0].arg)
  repeat = count//source_count if source_count and count%source_count == 0 and mode == "nonzero" else 1
  if not 1 <= repeat <= 8: return None
  def parse(term:UOp) -> tuple[UOp, UOp]|None:
    _, predicate, load = parsed[term]
    return load, term.substitute({predicate:predicate.const_like(True)})
  if (prefix:=_prefix_load_rows(out_index, count, terms, dtypes.half, parse, repeat)) is None: return None
  source, rows = prefix

  vector_bytes, vector_lanes, matrix_lanes = _stripe_layout(count, count)
  if matrix_lanes > _MAX_EW_ELEMS_FP16: return None
  one, maximum, negative_maximum, values = range(4)
  temps = tuple(range(4, 15)); int_tiles = 15
  scratch = tuple(RKScratch(_int32_tiles_bytes(count) if slot == int_tiles else _scratch_bytes(matrix_lanes))
                  for slot in range(int_tiles+1))
  gathers = _stripe_gathers(source.arg.slot, values, count, rows, vector_lanes)
  def arg(slot:int, addend:int=0) -> RKArg: return RKArg(RKBufferKind.SCRATCH, slot, addend)
  ops:list[RKEWOp] = []
  if mode == "positive": mask = _ew_ieee_positive_mask(ops, arg, values, (one, maximum, negative_maximum), temps, matrix_lanes)
  else:
    ...
    mask = arg(mask_slot)
  reduced = _reduce_rows(ops, [RKArg(mask.kind, mask.index, row*vector_bytes) for row in range(count)], count, _EW_CFG[Ops.ADD])
  ...
  return RKImage(RKTarget.RK3588, scratch, struct.pack("<eee", 1.0, 65504.0, -65504.0),
...
def _cumulative_index_image(out_slot:int, count:int, candidate_plans:tuple[RKGather, ...],
  ...
  coordinate_bits = tuple(tuple(_fp16_bits(limit-(candidate_coords[candidate][dst]
  ...
  ew_ops.append(RKEWOp(args(diff), args(zero if raw_weight else 2), selected, count, _EW_CFG[Ops.SUB]) if first_tie else
...
def _dynamic_fp16_gather_image(out_slot:int, count:int, indices:tuple[RKDynamicIndex, ...], plans:tuple[RKGather, ...],
                               ...
                               alternate_coordinates:tuple[tuple[tuple[int, ...], ...], ...]|None=None) -> RKImage|None:
  ...
  alternates = None if alternate_coordinates is None else tuple(tuple(tuple((candidate,)*count for candidate in axis)
    for axis in alternatives) for alternatives in alternate_coordinates)
  if (equality:=_int32_equality_matrix(equality_inputs, count, alternates)) is None: return None
  rows, matrix_lanes = len(plans), equality.matrix_lanes
  plan_offsets = tuple(_plan_offsets(plan) for plan in plans)
  compact_offsets = tuple(sorted({offset for row in plan_offsets for offset in row}))
  compact_index = {offset:i for i,offset in enumerate(compact_offsets)}
  compact_rows = tuple(tuple(compact_index[offset] for offset in row) for row in plan_offsets)
  compact_lanes = len(compact_offsets)
  next_slot = len(equality.scratch_sizes)
  raw_value = (next_slot, next_slot+1); compact_value = (next_slot+2, next_slot+3)
  matrix_value = (next_slot+4, next_slot+5)
  selected_low, selected_high, int_low, int_high = range(next_slot+6, next_slot+10)
  scratch_sizes = list(equality.scratch_sizes) + [_scratch_bytes(matrix_lanes)] * 10
  scratch_sizes[equality.tiles] = _int32_tiles_bytes(max(matrix_lanes, compact_lanes,
    *(index_count for _,index_count,_ in indices)))
  for slot in raw_value: scratch_sizes[slot] = compact_lanes*4
  for slot in compact_value: scratch_sizes[slot] = _scratch_bytes(compact_lanes)
  scratch_sizes[int_low] = scratch_sizes[int_high] = count*2

  if len({plan.src_index for plan in plans}) != 1: return None
  gathers = list(equality.gathers + _raw_fp16_gathers(plans[0].src_index, raw_value, compact_lanes,
                 (compact_offsets,), compact_lanes))
  mid_gathers = list(equality.mid_gathers)
  mid_gathers.extend(RKGather(src, dst, count, offsets=row, dst_addend=candidate*equality.vector_lanes,
    src_kind=RKBufferKind.SCRATCH) for src,dst in zip(compact_value, matrix_value)
    for candidate,row in enumerate(compact_rows))

  def arg(slot:int, addend:int=0) -> RKArg: return RKArg(RKBufferKind.SCRATCH, slot, addend)
  ops = list(equality.pre_ops)
  _append_byte_conversions(ops, arg, raw_value, compact_value, equality.tiles, compact_lanes)
  ops.extend(equality.mask_ops)
  ops.extend((RKEWOp(arg(selected_low), arg(matrix_value[0]), equality.mask, matrix_lanes,
                     _EW_CFG[Ops.MUL], submit_barrier=True, stateful=True),
              RKEWOp(arg(selected_high), arg(matrix_value[1]), equality.mask, matrix_lanes,
                     _EW_CFG[Ops.MUL], submit_barrier=True, stateful=True)))
  ...
  post_gathers = _raw_fp16_output(ops, arg, (low, high), count, equality.tiles,
                                  (int_low, int_high), out_slot, int16_output=True)
  return RKImage(RKTarget.RK3588, tuple(RKScratch(size) for size in scratch_sizes), struct.pack("<e", 1.0),
                 gathers=tuple(gathers), ew_ops=tuple(ops), mid_gathers=tuple(mid_gathers),
...
def _full_fp16_predicate_count(expr:UOp, out_index:UOp, count:int, predicate:Callable[[UOp], UOp|None],
                               max_scale:int=1) -> tuple[UOp, int]|None:
  """Prove an unrolled sum covers every FP16 source predicate once with one uniform optional scale."""
  ...
  try:
    for term in terms:
      ...
      param = _root_param(load.src[0])
      if (param is None or param.dtype.scalar() is not dtypes.half or param.src[0].op is not Ops.CONST or
          source is not None and param.arg.slot != source.arg.slot): return None
      source = param
  ...
  if (source is None or not scales or len(set(scales)) != 1 or not 1 <= scales[0] <= max_scale or
      len(terms) != int(source.src[0].arg) or sorted(offsets) != list(range(len(terms)))): return None
  return source, scales[0]
...
def _lower_fixed_fp16_nonzero(output:RKOutput) -> RKImage|None:
  ...
  try:
    if _eval_cast(fill_value, dtypes.half) != fill_value: return None
  except (OverflowError, struct.error): return None
  totals = [(u, u.src[1], info) for u in root.toposort() if u.op is Ops.CMPLT and u.src[0].key == out_index.key and
            (info:=_full_fp16_predicate_count(u.src[1], out_index, count, _nonzero_load, 8)) is not None]
  if len(totals) != 1: return None
  _, total_expr, (source, rank) = totals[0]
  source_count, coordinate_count = int(source.src[0].arg), int(source.src[0].arg)*rank
  if not 1 <= coordinate_count <= _FP16_EXACT_INTEGER: return None
  int_loads = [u for u in root.toposort() if u.op is Ops.LOAD and u.dtype.scalar() is dtypes.int]
  source_loads = {u.key for u in total_expr.toposort() if u.op is Ops.LOAD}
  if len(int_loads) != 1 or {u.key for u in root.toposort() if u.op is Ops.LOAD and u.dtype.scalar() is dtypes.half} != source_loads:
    return None
  index_load = int_loads[0]
  ...
  try:
    coordinate_rows = tuple(_static_int_vector(out_index, root.substitute({total_expr:total_const, index_load:index_load.const_like(i)}), count)
  ...
  try:
    if any(_eval_cast(value, dtypes.half) != value for row in coordinate_rows for value in row): return None
  except (OverflowError, struct.error): return None
...
def _lower_fixed_fp16_masked_select(output:RKOutput) -> RKImage|None:
  ...
  if root.op is Ops.WHERE and len(root.src) == 3: condition, selected, fill = root.src
    ...
    conditions = [u for u in root.src[2].toposort() if u.op is Ops.CMPLT and u.src[0].key == out_index.key and
                  _full_fp16_predicate_count(u.src[1], out_index, count, _positive_half_load) is not None]
  ...
  if (condition.op is not Ops.CMPLT or condition.src[0].key != out_index.key or
    ...
    return None
  if (count_info:=_full_fp16_predicate_count(condition.src[1], out_index, count, _positive_half_load)) is None: return None
  source, _ = count_info
  source_count = int(source.src[0].arg)
  ...
  value_matrix = (45, 46)
  scratch_sizes = [_scratch_bytes(matrix_lanes)] * 47
...
def _lower_dynamic_fp16_gather(output:RKOutput) -> RKImage|None:
  """Recognize Tinygrad's bounds-masked FP16 load addressed by one dynamic INT32 index."""
...
def _lower_unrolled_multi_fp16_fancy_index(output:RKOutput) -> RKImage|None:
  ...
  for term in terms:
    if term.op is not Ops.WHERE or term.dtype.scalar() is not dtypes.half: return None
    loads = [arm for arm in term.src[1:] if arm.op is Ops.LOAD and arm.dtype.scalar() is dtypes.half and
  ...
  if (any(param is None or param.dtype.scalar() is not dtypes.half or
...
def _lower_unrolled_max_unpool(output:RKOutput) -> RKImage|None:
  ...
  return _max_unpool_image(out_param.arg.slot, count, out_spatial, source_count, index_param.arg.slot, plans, parsed[0][3])

def _lower_loop_max_unpool(uops:list[UOp], output:RKOutput) -> RKImage|None:
  ...
  if (where.src[0].op is not Ops.CMPNE or where.src[1].op is not Ops.CONST or float(where.src[1].arg) != 0.0 or
      (selected:=_strip_cast(where.src[2])).op is not Ops.LOAD or selected.dtype.scalar() is not dtypes.half): return None
  ...
  if (index_param is None or value_param is None or index_param.src[0].op is not Ops.CONST or value_param.src[0].op is not Ops.CONST or
      index_param.dtype.scalar() is not dtypes.int or value_param.dtype.scalar() is not dtypes.half): return None
  source_count = int(index_param.src[0].arg)
  ...
  return _max_unpool_image(out_param.arg.slot, count, out_spatial, source_count, index_param.arg.slot, plans)
...
def lower_ew(uops:list[UOp]) -> RKImage:
  ...
  if (half_output:=_output_store(uops, dtypes.half)) is not None:
    ...
    if (unrolled_fancy:=_lower_unrolled_multi_fp16_fancy_index(half_output)) is not None: return unrolled_fancy
