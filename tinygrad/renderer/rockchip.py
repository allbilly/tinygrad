...
def _map_image_args(image:RKImage, fn:Callable[[RKArg], RKArg], *, map_value_src:bool=True) -> RKImage:
  def gather(value:RKGather) -> RKGather:
    src = fn(RKArg(value.src_kind, value.src_index)) if map_value_src or not value.values else RKArg(value.src_kind, value.src_index)
    dst = fn(RKArg(value.dst_kind, value.dst_index))
...
def _reuse_linear_scratch(image:RKImage, constant_slots:dict[bytes, int]) -> RKImage:
  ...
  def gather_args(gather:RKGather) -> tuple[RKArg, ...]:
    source = () if gather.values else (RKArg(gather.src_kind, gather.src_index),)
    return source+(RKArg(gather.dst_kind, gather.dst_index),)
  ...
  schedule += [gather_args(gather) for gather in mid_by_point.get(len(image.ew_ops), ())] + [gather_args(gather) for gather in image.post_gathers]
  ...
  image = _map_image_args(image, remap_arg, map_value_src=False)
...
def _encode_gather(g:RKGather) -> bytes:
  kind = 3 if g.partial else 2 if g.values else 1 if g.offsets else 0
  return _GATHER.pack(g.dst_index, g.src_index, g.count, kind, len(g.axes), g.itemsize, int(g.dst_kind), int(g.src_kind),
...
def encode_image(image:RKImage) -> bytes:
  ...
  for g in gathers: out += _encode_gather(g)
...
def decode_image(blob:bytes) -> RKImage:
  ...
  for _ in range(nop):
    ...
    if op_flags & ~0xff or op_flags & 0x20 and not op_flags & 0x08 or op_flags & 0xc0 and op_flags & 0x18 and not int16_to_int32:
...
def patch_stage(stage:RKStage, address:Callable[[RKBufferKind, int], int]) -> tuple[int, ...]:
  ...
  for reloc in stage.relocs:
    ...
    commands[reloc.word] = (word & ~0xffffffff0000) | (((address(reloc.arg.kind, reloc.arg.index) + reloc.arg.addend) & 0xffffffff) << 16)
...
_EW_CFG = {Ops.ADD:_EW_CFG_COMMON|_EW_RELU_BYPASS|_EW_ALU_ADD, Ops.SUB:_EW_CFG_COMMON|_EW_RELU_BYPASS|_EW_ALU_SUB,
  Ops.MUL:_EW_CFG_COMMON|_EW_RELU_BYPASS|_EW_OP_CVT_BYPASS|_EW_OP_TYPE_MUL, Ops.MAX:_EW_CFG_COMMON|_EW_RELU_BYPASS,
...
def _int16_low_bytes(source:RKArg, out_slot:int, count:int, stride:int=2) -> RKGather:
  return RKGather(source.index, out_slot, count, base=source.addend, axes=((1, count, stride),), dst_kind=RKBufferKind.ARG,
...
def _static_cast(value, dtype:DType, vector:bool=False):
  ...
  scalar = dtype.scalar()
  return bool(value) if scalar is dtypes.bool else int(value) if scalar in dtypes.ints else float_to_fp16(value) if scalar is dtypes.half else \
    struct.unpack("<f", struct.pack("<f", float(value)))[0] if scalar is dtypes.float else float(value)

_StaticValue = int|float|bool|np.ndarray
_STATIC_SCALAR_ALU = {Ops.RECIPROCAL, Ops.TRUNC, Ops.ADD, Ops.MUL, Ops.SUB, Ops.CDIV, Ops.CMOD, Ops.FLOORDIV, Ops.FLOORMOD,
                      Ops.MAX, Ops.CMPLT, Ops.CMPNE, Ops.AND, Ops.OR, Ops.XOR}
def _static_alu(op:Ops, dtype:DType, values:tuple, vector:bool):
  if vector:
    lhs = values[0]
    if op in (Ops.CDIV, Ops.CMOD):
      with np.errstate(divide="ignore", invalid="ignore"): quotient = np.where(values[1] != 0, np.trunc(lhs / values[1]), 0)
      value = quotient if op is Ops.CDIV else lhs-quotient*values[1]
    elif op in (Ops.FLOORDIV, Ops.FLOORMOD):
      quotient = np.zeros(np.broadcast_shapes(lhs.shape, values[1].shape), dtype=np.result_type(lhs, values[1]))
      np.floor_divide(lhs, values[1], out=quotient, where=values[1] != 0)
      value = quotient if op is Ops.FLOORDIV else lhs-quotient*values[1]
    ...
    if op is Ops.RECIPROCAL: value = 1.0 / float(values[0])
    elif op is Ops.TRUNC: value = int(values[0])
    else:
...
def _eval_expr(u:UOp, env:Mapping[UOp, _StaticValue], cache:dict[UOp, _StaticValue], vector:bool=False) -> _StaticValue:
  if u in cache: return cache[u]
  if u.op is Ops.CONST: ret = _static_cast(u.arg, u.dtype, vector)
  elif u.op in (Ops.RANGE, Ops.SPECIAL): ret = _static_cast(env[u], u.dtype, True) if vector else env[u]
  elif u.op is Ops.PARAM: raise RuntimeError("RKPLAN_REJECT:dynamic_static_expr")
  elif u.op is Ops.AFTER: ret = _eval_expr(u.src[0], env, cache, vector)
  elif u.op is Ops.CAST: ret = _static_cast(_eval_expr(u.src[0], env, cache, vector), u.dtype, vector)
  ...
  else: ret = _static_alu(u.op, u.dtype, tuple(_eval_expr(src, env, cache, vector) for src in u.src), vector)
  cache[u] = ret
  return ret

def _eval_int(u:UOp, env:dict[UOp, int], cache:dict[UOp, int|float|bool]|None=None) -> int:
  return int(_eval_expr(u, env, typing_cast(dict[UOp, _StaticValue], {} if cache is None else cache)))

def _eval_vector(u:UOp, env:Mapping[UOp, np.ndarray|int], cache:dict[UOp, np.ndarray]) -> np.ndarray: return np.asarray(
  _eval_expr(u, env, typing_cast(dict[UOp, _StaticValue], cache), True))
...
def _try(o,d,f,*a)->RKImage|None: return None if (o:=_admit(o,d)) is None else f(o,*a)
def _generic_output(o): return o if o is not None and o[1].dtype.scalar() is not dtypes.uchar and len(o[0].src) == 2 else None
def _coords(o): return next((x for d in (dtypes.int16,dtypes.int) if
  (x:=_lower_bounded_integer_predicate_coordinates(o,d)) is not None),None)
def _accurate(o,r,s,p,n): return not r and (s is None or p) and len(n) <= _MAX_OPTIONAL_RECIPE_NODES and not _has_runtime_address(o[4])
...
def _loop_reduction_match(output:RKOutput) -> RKLoopReduction|None:
  ...
  if out.src[0].op is not Ops.CONST or store.src[0].op is not Ops.INDEX: return None
  ...
  if (value_root.op is Ops.SQRT and len(value_root.src) == 1 or
      value_root.op is Ops.FDIV and len(value_root.src) == 2 and value_root.src[0].op is Ops.CONST and
      float(value_root.src[0].arg) == 1.0 and _local_load(value_root.src[1]) is not None or len(local_values) == 1 and any(
      node.op is Ops.CONST and node.dtype.scalar() in (dtypes.half, dtypes.float) and abs(float(node.arg)-1/3) < 1e-6
      for node in value_root.toposort())): return None
  value = value_root
...
def _gather_offsets(out_index:UOp, load_index:UOp, gate:UOp|None, count:int) -> tuple[int, ...]:
  ...
  values = src if gate is None else np.where(active, src, -1)
...
def _contiguous_output(out_index:UOp, count:int) -> bool:
  ranges, affine = _index_ranges(out_index), typing_cast(tuple[int, dict[UOp, int]]|None, _linear_index(out_index))
  return affine is not None and affine[0] == 0 and set(affine[1]) == set(ranges) and _affine_output_axes(affine, count) is not None

def _repeated_index_offsets(indices:Iterable[UOp], vector_env:dict[UOp, np.ndarray], count:int) -> tuple[tuple[int, ...], tuple[int, ...]]:
  cache:dict[UOp, np.ndarray] = {}
  values = tuple(np.broadcast_to(_eval_vector(index.src[1], vector_env, cache), count).astype(np.int64) for index in indices)
  if any(len(np.unique(value-values[0])) != 1 for value in values): raise RuntimeError("nonuniform repeated index")
  return tuple(int((value-values[0])[0]) for value in values), tuple(int(offset) for value in values for offset in value)
...
def _gather_plan(src_index:int, dst_index:int, out_index:UOp, load_index:UOp, gate:UOp|None, count:int, fill_bits:int=0) -> RKGather:
  ...
  if gate is None and out_affine is not None and load_affine is not None and out_affine[0] == 0 and \
     (output_axes:=_affine_output_axes(out_affine, count)) is not None and all(r in out_affine[1] for r in load_affine[1]):
  ...
  if gate is None and out_affine is not None and out_affine[0] == 0 and \
     (load_divided:=typing_cast(tuple[int, dict[tuple[UOp, int], int]]|None, _linear_index(load_index, True))) is not None and \
     (output_axes:=_affine_output_axes(out_affine, count)) is not None and \
...
def _typed_load_plan(load:UOp, dtype:DType, out_index:UOp, count:int, *, strip_cast:bool=False,
  ...
  if param is None or param.dtype.scalar() is not dtype or not param.src or param.src[0].op is not Ops.CONST or \
...
def _gather_cache_key(plans:Iterable[RKGather]) -> tuple: return tuple((p.src_index, p.count, p.base, p.axes, p.offsets,
  p.fill_bits, p.values, p.partial, p.dst_stride, p.dst_addend, p.itemsize, p.src_kind) for p in plans)
...
def _tag_precise_adds(root:UOp) -> UOp:
  ...
  for node in root.toposort():
    tagged[node] = node.replace(src=tuple(tagged[src] for src in node.src), arg=_NATIVE_PRECISE_ADD if node.op is Ops.ADD else node.arg)
  return tagged[root]
...
def _product_residual_stages(product:RKArg, lhs:RKArg, rhs:RKArg, lhs_high:RKArg, rhs_high:RKArg,
                             error:RKArg, splitter:RKArg) -> tuple[tuple[RKArg, RKArg, RKArg, Ops], ...]:
  """Return the exact FP16 TwoProduct physical stage sequence."""
  return ((product, lhs, rhs, Ops.MUL),
...
def _reduction_store(store:UOp, out:UOp, index:UOp, lanes:int, value:UOp) -> tuple[UOp, UOp]:
...
def _reduction_post_parts(value:UOp, output_dtype:DType) -> tuple[float, UOp|None, UOp|None]|None:
  ...
  return None if not local_refs else (1.0, value, next((u for u in local_refs if u.dtype.scalar() is output_dtype.scalar()), local_refs[-1]))
...
def _reduce_mapped_rows(ops:list[RKEWOp], active:list[int], count:int, arena:Callable[[int], RKArg], temporary:Callable[[int], RKArg],
                        out:RKArg, compensated_limit:int, kahan:bool, op_barriers:bool) -> RKArg:
  return _kahan_add(ops, active, count, arena, temporary, out, op_barriers=op_barriers) if kahan else \
    _compensated_add(ops, active, count, arena, temporary, out, op_barriers=op_barriers) if 2 <= len(active) <= compensated_limit else \
    _reduce_arena(ops, active, count, _EW_CFG[Ops.ADD], arena, out, op_barriers=op_barriers)
...
def _spaced_reduction(ops:list[RKEWOp], mid:list[RKGather], source:RKArg, count:int, allocate:Callable[[int], RKArg], cfg:int,
  ...
  stride = _reduction_stride(1)
  spaced = allocate(count*stride//2)
...
def _finish_mapped_add_reduction(mapped:RKImage, out_slot:int, rows:int, groups:int, post_scale:float,
  ...
  lanes = rows*groups
  scratch_shift, value_slot = 1, len(mapped.scratch)+1; ss,vs,os,kind = scratch_shift,value_slot,out_slot,RKBufferKind.SCRATCH
  def remap_arg(arg): return replace(arg,index=arg.index+ss) if arg.kind is kind else replace(arg,kind=kind,index=vs) if arg.index==os else arg
  mapped = _map_image_args(mapped, remap_arg)
  ops = list(mapped.ew_ops)
  arena_slot = value_slot+1
  stride, _, arena, active = _reduction_arena(rows, groups, arena_slot)
  ...
  gather_after = len(mapped.ew_ops)
  ...
  reduced = _reduce_mapped_rows(ops, active, rows, arena, temporary, destination, compensated_limit, kahan, op_barriers)
  if post_scale != 1.0:
  ...
  mapped_mid = mapped.mid_gathers
  mid = mapped_mid+tuple(replace(gather, after=len(mapped.ew_ops)) for gather in mid)
  return replace(mapped, scratch=scratch, constants=struct.pack("<e", post_scale)+mapped.constants, ew_ops=tuple(ops), mid_gathers=mid,
...
def _append_mapped_product_residual(mapped:RKImage, out_slot:int, out_index:UOp, product:UOp, lanes:int) -> RKImage|None:
  ...
  if product.op is not Ops.MUL or product.arg is not None or mapped.post_gathers or mapped.host_scatters: return None
  ...
  if direct is None or expression is None or (parsed:=_typed_load_plan(direct, dtypes.half, out_index, lanes,
                                                                       strip_cast=True, require_default=True)) is None: return None
  rhs_gather = parsed.gather
  ...
  rhs_gather = replace(rhs_gather, dst_index=rhs.index)
  product_arg, error_arg = RKArg(RKBufferKind.ARG, out_slot), RKArg(RKBufferKind.ARG, out_slot, lanes*2)
  stages = _product_residual_stages(product_arg, lhs, rhs, lhs_high, rhs_high, error_arg, splitter)
  ops = mapped.ew_ops+_ew_ops(stages, lanes, lambda i:i == 0, True)
...
def _append_inplace_image(first:RKImage, second:RKImage) -> RKImage|None:
  ...
  fc,sc,kind = len(first.constants)//2, len(second.constants)//2, RKBufferKind.SCRATCH
  def first_arg(arg): return replace(arg, index=arg.index+sc) if arg.kind is kind and arg.index >= fc else arg
  def second_arg(arg): return arg if arg.kind is not kind else replace(arg, index=fc+arg.index if arg.index < sc else len(first.scratch)+arg.index)
  first, second = _map_image_args(first, first_arg), _map_image_args(second, second_arg)
  second_ops = list(second.ew_ops)
  second_ops[0] = replace(second_ops[0], submit_barrier=True, stateful=True)
  split = len(first.ew_ops)
  second_mid = tuple(replace(gather, after=split) for gather in second.gathers)+tuple(
    replace(gather, after=split+(gather.after if gather.after >= 0 else second.gather_after)) for gather in second.mid_gathers)
...
def _lower_post_image(store:UOp, out_index:UOp, root:UOp, substitutions:dict[UOp, UOp]) -> RKImage|None:
  ...
  post_value = root.substitute(substitutions).substitute(range_subs)
...
def _append_reduction_post(reduced:RKImage, uops:list[UOp], store:UOp, out_index:UOp,
  ...
  post = replace(post, ew_ops=tuple(replace(x, lhs=x.rhs, rhs=x.lhs) if x.dst == x.rhs and x.ew_cfg in commutative else x for x in post.ew_ops))
...
def _lower_mapped_add_loop_reduction(output:RKOutput, uops:list[UOp]) -> RKImage|None:
  ...
  _, fake_store = _reduction_store(store, out, flat.alu(Ops.MUL, flat.const_like(rows)).alu(Ops.ADD, out_index), lanes, term)
  mapped = _lower_uop_program(list(UOp(Ops.SINK, src=(fake_store,)).toposort()), vectorize_reductions=False, recipes_ready=True)
  ...
  return _append_reduction_post(reduced, uops, store, out_index, post_root, post_local, rows, out_slot)

def _lower_vectorized_unrolled_add_reduction(output:RKOutput, uops:list[UOp]) -> RKImage|None:
  ...
  if (scaled:=_scaled_add_terms(root)) is None: return None
  ...
  if not (root.op is Ops.CAST and root.src[0].op is Ops.MUL) and len(input_slots) < 2: return None
  ...
  for leaf in leaves:
    ...
    try:
      offsets, absolute_offsets = _repeated_index_offsets((item[0] for item in concrete), vector_env, count)
    except (RuntimeError, ValueError): return None
    length, run = len(offsets), next((i for i,value in enumerate(offsets) if value != offsets[0]), len(offsets))
    period = next((size for size in range(2, length) if length%size == 0 and offsets == offsets[:size]*(length//size)), length)
    ...
    if blocks == tuple(blocks[0]+i*stride for i in range(len(blocks))):
      mapped_index = concrete[0][0].src[1].alu(Ops.ADD, index_lane.alu(Ops.MUL, lane.const_like(stride)).alu(Ops.ADD, lane.const_like(blocks[0])))
      source_param = params[0]
    else:
      if any(not 0 <= offset < int(params[0].src[0].arg) for offset in absolute_offsets): return None
  ...
  fake_index, _ = _reduction_store(store, out, lane.alu(Ops.MUL, lane.const_like(count)).alu(Ops.ADD, out_index), count*len(terms), vector)
  ...
  def lower_map(root:UOp) -> RKImage|None:
    ...
    return _lower_uop_program(map_uops, vectorize_reductions=False, recipes_ready=True)
  ...
  if precise_product:
    if (residual:=_append_mapped_product_residual(mapped, out.arg.slot, fake_index.src[1], vector, count*len(terms))) is None:
      if (mapped:=lower_map(vector)) is None: return None
  ...
  finished = _finish_mapped_add_reduction(mapped, out.arg.slot, count, mapped_groups, post_scale, op_barriers=precise_product, kahan=precise_product)
...
def _lower_vectorized_mul_add_reduction(output:RKOutput, uops:list[UOp]) -> RKImage|None:
  ...
  for term in terms:
    ...
    for src in term.src:
      if (parsed_load:=_typed_load_plan(src, dtypes.half, out_index, rows, strip_cast=True, require_default=True)) is None: return None
      operands.append((parsed_load.param, parsed_load.gather))
  ...
  for start in range(0, lanes, chunk_lanes):
    ...
    stages = _product_residual_stages(product, lhs, rhs, lhs_high, rhs_high, error, splitter)
    mapped_ops.extend(_ew_ops(stages, count, lambda i: i == 0 and bool(start), True))
  ...
  return _append_reduction_post(finished, uops, store, out_index, root, summed, rows, out.arg.slot)
...
def _flatten_binary(root:UOp, op:Ops, plain:bool=False) -> list[UOp]: return list(_iter_binary(root, op, plain=plain))
...
def _scaled_add_terms(root:UOp) -> tuple[tuple[UOp, ...], float]|None:
  ...
  return (tuple(_strip_cast(term) for term in _flatten_binary(root, Ops.ADD)), scale) if root.op is Ops.ADD and math.isfinite(scale) else None
...
class _RKBuilder:
  ...
  def ew(self, lhs:RKArg, rhs:RKArg, count:int, cfg:int, dst:RKArg|None=None, size:int|None=None, **flags) -> RKArg:
    if dst is None: dst = self.scratch(count*2 if size is None else size)
    self.ops.append(RKEWOp(dst, lhs, rhs, count, cfg, **flags)); return dst
  def i16(self, lhs:RKArg, rhs:RKArg, count:int, cfg:int, dst:RKArg|None=None) -> RKArg:
    return self.ew(lhs, rhs, count, cfg, dst, **_INT16_EW)
...
def _reduce_arena(ops:list[RKEWOp], active:list[int], count:int, cfg:int, arena:Callable[[int], RKArg],
  ...
  while len(active) > 1:
    ...
    for i in range(0, len(active)-1, 2):
      ...
      ops.extend(_ew_ops(((dst, arena(lhs), arena(rhs), cfg | (_EW_STAGE_FP32_OUT if fp32_out and final else 0)),), count,
...
def _kahan_add(ops:list[RKEWOp], active:list[int], count:int, arena:Callable[[int], RKArg],
  ...
  stages = itertools.chain(
    ...
    *(((adjusted, arena(offset), correction, Ops.SUB), (updated, total, adjusted, Ops.ADD),
       ...
       (total, updated, updated, Ops.MAX)) for offset in active[1:]), ((out, total, total, Ops.MAX),))
  ops.extend(_ew_ops(stages, count, op_barriers, op_barriers))
  return out
...
def _lower_fp16_uint8_cast(output:RKOutput) -> RKImage|None:
  ...
  root, zero = output[4], UOp.const(0.0, dtypes.half)
  source = _typed_cast_source(root, dtypes.uchar, dtypes.half)
...
def _int16_byte_bits(ops:list[RKEWOp], allocate:Callable[[], RKArg], constants:dict[int, RKArg], value:RKArg, lanes:int) -> tuple[RKArg, ...]:
  ...
  for bit in range(7, 0, -1):
    ...
    ops.extend(_ew_ops(((delta, remainder, constants[(1<<bit)-1], Ops.SUB),
      (positive, delta, constants[0], Ops.MAX), (flag, positive, constants[1], _EW_CFG_MIN),
      (scaled, flag, constants[1<<bit], Ops.MUL), (next_remainder, remainder, scaled, Ops.SUB)), lanes, **_INT16_EW))
...
def _lower_raw_fp16_bitcast(output:RKOutput) -> RKImage|None:
  ...
  lanes:dict[int, tuple[UOp, tuple[int, ...]]] = {}
  for term in packed.src:
    ...
    lanes[shift] = (parsed.param, parsed.offsets)
  if len(lanes) != 2 or lanes[0][0].arg.slot != lanes[16][0].arg.slot: return None
  low, high = (lanes[shift][1] for shift in (0, 16))
  if any(offset < 0 or offset+1 >= int(lanes[0][0].src[0].arg) or offset & 1 or high[i] != offset+1 for i,offset in enumerate(low)): return None
  gather = RKGather(lanes[0][0].arg.slot, out_param.arg.slot, count, offsets=tuple(offset//2 for offset in low),
...
def _int32_less_mask(ops:list[RKEWOp], allocate:Callable[[], RKArg], constants:dict[int, RKArg],
  ...
  def biased_sign(value:RKArg) -> RKArg:
    ...
    ops.extend(_ew_ops(((delta, value, constants[127], Ops.SUB), (positive, delta, constants[0], Ops.MAX),
      (high, positive, constants[1], _EW_CFG_MIN), (scaled, high, constants[256], Ops.MUL),
      (biased, value, constants[128], Ops.ADD), (biased, biased, scaled, Ops.SUB)), lanes, **_INT16_EW))
    return biased
...
def _candidate_gather(src:int, dst:int, rows:tuple[tuple[int, ...], ...], lanes:int) -> RKGather:
  ...
  affine = lanes%n == 0 and all(value == base+r*dy+c*dx for r,row in enumerate(rows) for c,value in enumerate(row))
  return RKGather(src, dst, len(rows)*lanes, base, ((1, n, dx), (lanes, len(rows), dy)), dst_stride=2, itemsize=1) if affine else \
...
def _negative_normalized_index(root:UOp) -> tuple[UOp, int]|None:
  ...
  addition = _const_operand(root.src[1], Ops.ADD)
  return (index, int(addition[1].arg)) if index.op is Ops.LOAD and index.dtype.scalar() is dtypes.int and root.src[2].key == index.key and \
    addition is not None and addition[0].key == index.key and int(addition[1].arg) > 0 else None
...
def _bounded_index_gate(gate:UOp, bounded:UOp, limit:int) -> bool:
  ...
  nodes = gate.toposort()
  upper = any(u.op is Ops.CMPLT and u.src[0].key == bounded.key and u.src[1].op is Ops.CONST and int(u.src[1].arg) == limit for u in nodes)
...
RKBoundedAxis = tuple[UOp, UOp, int, bool]
def _bounded_dynamic_axes(address:UOp, gate:UOp) -> tuple[RKBoundedAxis, ...]|None:
  """Prove every dynamic INT32 address axis has one exact static extent and bounds gate."""
  address_nodes, gate_nodes = address.toposort(), gate.toposort()
  normalized = tuple((u, *parsed) for u in address_nodes if (parsed:=_negative_normalized_index(u)) is not None)
  ...
  axes:list[RKBoundedAxis] = []
  for dynamic in loads:
    if dynamic.key in normalized_by_load: root, extent, wrapped = *normalized_by_load[dynamic.key], True
    else:
      limits = {int(u.src[1].arg) for u in gate_nodes if u.op is Ops.CMPLT and u.src[0].key == dynamic.key and
...
def _native_int16_byte_mask(builder:_RKBuilder, index_slot:int, index_offsets:tuple[int, ...]|tuple[tuple[int, ...], ...],
                            coordinate_sets:tuple[RKCoordinateRows, ...], count:int, vector_lanes:int) -> RKArg|None:
  ...
  for coordinates in coordinate_sets:
    ...
    for byte in range(4):
      ...
      dynamic_rows = tuple(tuple(offset*4+byte for offset in offsets) for offsets in offset_rows)
      builder.gathers.append(_candidate_gather(index_slot, dynamic.index, dynamic_rows, vector_lanes))
      values = tuple((value >> (byte*8)) & 0xff for row in coordinates for value in (*row, *((0,)*(vector_lanes-count))))
      builder.gathers.append(RKGather(index_slot, static.index, matrix_lanes, values=values, itemsize=2))
      builder.i16(dynamic, static, matrix_lanes, _EW_CFG[Ops.SUB], diff)
      builder.i16(diff, diff, matrix_lanes, _EW_CFG_ABS, magnitude)
      builder.i16(magnitude, one, matrix_lanes, _EW_CFG_MIN, unequal)
...
def _native_integer_nonzero_mask(builder:_RKBuilder, source_slot:int, rows:tuple[tuple[int, ...], ...],
                                 count:int, vector_lanes:int, itemsize:int=4) -> RKArg|None:
  ...
  byte_masks = tuple(builder.scratch(matrix_lanes*2) for _ in range(itemsize))
  builder.gathers.append(RKGather(source_slot, one.index, matrix_lanes, values=(1,)*matrix_lanes, itemsize=2))
  for byte,value in enumerate(byte_masks):
    builder.gathers.extend(RKGather(source_slot, value.index, count,
  ...
  return _reduce_byte_masks(builder, byte_masks, matrix_lanes, _EW_CFG[Ops.MAX], in_place=True)
...
def _prefix_valid(builder:_RKBuilder, total:RKArg, coordinate:RKArg, zero:RKArg, one:RKArg, count:int) -> tuple[RKArg, RKArg]:
  ...
  builder.i16(total, coordinate, count, _EW_CFG[Ops.SUB], delta)
  builder.i16(delta, zero, count, _EW_CFG[Ops.MAX], positive)
  builder.i16(positive, one, count, _EW_CFG_MIN, valid)
  builder.i16(one, valid, count, _EW_CFG[Ops.SUB], remaining)
  return valid, remaining

def _select_int16_rows(builder:_RKBuilder, equal:RKArg, values:tuple[RKArg, ...], fills:tuple[RKArg, ...],
                       valid:RKArg, remaining:RKArg, rows:int, count:int, vector_bytes:int, result_size:int) -> tuple[RKArg, ...]:
  results:list[RKArg] = []
  for value,fill in zip(values, fills):
    selected = builder.scratch(rows*vector_bytes)
    guarded, fill_part, result = (builder.scratch(result_size) for _ in range(3))
    builder.i16(value, equal, rows*vector_bytes//2, _EW_CFG[Ops.MUL], selected)
    reduced = _reduce_rows(builder.ops, [replace(selected, addend=row*vector_bytes) for row in range(rows)], count, _EW_CFG[Ops.ADD], int16=True)
    builder.i16(reduced, valid, count, _EW_CFG[Ops.MUL], guarded)
    builder.i16(fill, remaining, count, _EW_CFG[Ops.MUL], fill_part)
    results.append(builder.i16(guarded, fill_part, count, _EW_CFG[Ops.ADD], result))
...
def _lower_bounded_integer_predicate_coordinates(output:RKOutput, dtype:DType=dtypes.int) -> RKImage|None:
  ...
  builder = _RKBuilder(64)
  _, source_vector_lanes, _ = _stripe_layout(1, source_count)
  source_rows = tuple((lane,) for lane in range(source_count))
  if (source_mask:=_native_integer_nonzero_mask(builder, source.arg.slot, source_rows, 1, source_vector_lanes, dtype.itemsize)) is None: return None
  ...
  gather_after = len(builder.ops)
  total_vector, output_coordinate, zero, one, coordinate_matrix = (builder.scratch(matrix_lanes*2) for _ in range(5))
  ...
  valid, remaining = _prefix_valid(builder, total_vector, output_coordinate, zero, one, count)
  result, = _select_int16_rows(builder, equal, (coordinate_matrix,), (fill_value_arg,), valid, remaining,
                               coordinate_count, count, vector_bytes, matrix_lanes*2)
...
def _lower_dynamic_typed_load(output:RKOutput, dtype:DType=dtypes.half) -> RKImage|None:
  ...
  if root.op is Ops.WHERE and len(root.src) == 3:
    ...
    if dtype is dtypes.half:
      ...
      same_default = float(load.src[1].arg) == float(fill.arg)
  ...
  params = typing_cast(tuple[UOp, ...], tuple(_root_param(u.src[0]) if u.src and u.src[0].op is Ops.INDEX else None for u in loads))
  ...
  if bool_load is not None and (bool_load not in _flatten_binary(gate, Ops.AND) or bool_param is None or bool_param.dtype.scalar() is not
  ...
  pre_vl = _stripe_layout(count, 2)[1] if candidates > 1 else count
  if pre_vl > _MAX_EW_ELEMS_FP16: return None
  pre_blocks = ceildiv(candidates, _MAX_EW_ELEMS_FP16//pre_vl)
  if candidates > _MAX_STATIC_RANGE_ENVS or candidates > _MAX_DYNAMIC_SELECTOR_CELLS//count or any(size > _RKIMAGE_U16_MAX for size in (
  ...
  grouped_indices = tuple((param.arg.slot, offsets[::repeat]) for param,offsets in zip(params, index_offsets))
  grouped_gate = None if external is None else (external[0], external[1][::repeat])
  vector_lanes = _stripe_layout(group_count, 2)[1] if candidates > 1 else group_count
  if vector_lanes > _MAX_EW_ELEMS_FP16: return None
  block_rows = _MAX_EW_ELEMS_FP16//vector_lanes
  block_count = ceildiv(candidates, block_rows)
  ...
  for start in range(0, candidates, block_rows):
    stop = min(candidates, start+block_rows)
    rows, matrix_lanes = stop-start, (stop-start)*vector_lanes
    axis_masks:list[RKArg] = []
    for axis,((index_slot, offsets), values) in enumerate(zip(grouped_indices, coordinates)):
    ...
    for channel in range(repeat):
      ...
      for byte in range(dtype.itemsize):
        raw, selected = builder.scratch(matrix_lanes*2), builder.scratch(matrix_lanes*2)
        ...
        builder.i16(raw, mask, matrix_lanes, _EW_CFG[Ops.MUL], selected)
        result.append(_reduce_rows(builder.ops, [replace(selected, addend=row*vector_lanes*2) for row in range(rows)], group_count,
  ...
  if total_gate is not None:
    ...
    gather_after = len(builder.ops)
    total_vector, coordinate, zero, one = (builder.scratch(count*2) for _ in range(4))
    mid = (RKGather(total.index, total_vector.index, count, offsets=(total.addend//2,)*count, src_kind=RKBufferKind.SCRATCH),)
    builder.gathers.extend((RKGather(mask_param.arg.slot, coordinate.index, count, values=tuple(range(count)), itemsize=2),
                            RKGather(mask_param.arg.slot, zero.index, count, values=(0,)*count, itemsize=2),
                            RKGather(mask_param.arg.slot, one.index, count, values=(1,)*count, itemsize=2)))
    ...
    for byte,value in enumerate(results[0]):
      ...
      delta = builder.i16(value, fill_value, count, _EW_CFG[Ops.SUB])
      selected = builder.i16(delta, valid, count, _EW_CFG[Ops.MUL])
      builder.i16(fill_value, selected, count, _EW_CFG[Ops.ADD], value)
  terminal = tuple(RKGather(value.index, out_param.arg.slot, group_count,
    offsets=tuple(range(value.addend, value.addend+group_count*2, 2)), dst_stride=repeat*dtype.itemsize,
    dst_addend=channel*dtype.itemsize+byte, dst_kind=RKBufferKind.ARG, src_kind=RKBufferKind.SCRATCH, itemsize=1)
...
def _exact_int_range(root:UOp, cache:dict[UOp, tuple[int, int]|None]|None=None) -> tuple[int, int]|None:
  ...
  if valid and dtype.min <= (low:=int(root.vmin)) <= (high:=int(root.vmax)) <= dtype.max:
    cache[root] = (0, max(0, high)) if root.op is Ops.RANGE else (low, high)
  else: cache[root] = None
  return cache[root]
...
def _typed_load_gather(load:UOp, dtype:DType, out_index:UOp, count:int, fill_bits:int) -> RKGather:
  if (plan:=_typed_load_plan(load, dtype, out_index, count, fill_bits=fill_bits)) is None: raise _RKGenericReject
  return plan.gather
...
def _runtime_affine_index(u:UOp, out_index:UOp, count:int) -> tuple[UOp, UOp, int, int, int, int, int]|None:
  ...
  try:
    lane_offsets = _static_values(out_index, lane_index, count, int)
    zero = _static_values(out_index, u.substitute({load:load.const_like(0)}), count, int)
    one = _static_values(out_index, u.substitute({load:load.const_like(1)}), count, int)
  except RuntimeError: return None
  lane_offset = lane_offsets[0]
  if lane_offsets != tuple(lane_offset+lane for lane in range(count)) or not 0 <= lane_offset <= int(param.src[0].arg)-count: return None
...
def _fp32_add_has_product_terms(u:UOp) -> bool:
  ...
  return any(term.op is Ops.MUL and term.dtype.scalar() is dtypes.float or
    ...
    for term in _iter_binary(u, Ops.ADD, dtypes.float))
def _storage_product(u,r):
  return u.op is Ops.CAST and u.dtype.scalar() is dtypes.half and len(u.src)==1 and \
    u.src[0].dtype.scalar() is dtypes.float and _fp32_add_has_product_terms(u.src[0]) and \
    (u is not r or len(u.src[0].toposort())>64)
...
def _accurate_add_recipe(u:UOp) -> UOp:
  ...
  for x in _flatten_binary(u, Ops.ADD, plain=True):
    if (source:=_typed_cast_source(x, dtypes.half, dtypes.float)) is not None and source.op is Ops.ADD:
      terms.extend(_fp32_add_terms(source))
    else: terms.append(x)
...
class RKContext:
  ...
  def _materialized_slot(self, key:tuple, dtype:DType, layout:RKLayout, plan:RKGather, size:int|None=None) -> RKValue:
    ...
    return self.materialized_slots[key]

  def _static_slot(self, dtype:DType, layout:RKLayout, vector:tuple[int, ...]) -> RKValue:
    cached = self._materialized_slot(("static", layout, vector), dtype, layout,
  ...
  def _gather_slot(self, dtype:DType, layout:RKLayout, plan:RKGather, size:int, key:tuple|None=None) -> RKValue:
    return self._materialized_slot(("gather", layout, _gather_cache_key((plan,))) if key is None else ("gather", key), dtype, layout, plan, size)
  ...
  def _static_int32(self, u:UOp) -> RKValue:
    return self._static_slot(dtypes.int, RKLayout.INT32, tuple(x&0xffffffff for x in _static_values(self.out_index, u, self.count, int)))
  ...
  def _load(self, u:UOp, fill_override:int|None=None) -> RKValue:
    ...
    if default is not None and default.op is not Ops.CONST:
      ...
      if fallback.layout is not expected or fallback.count != self.count or \
         schedule != (len(self.ew_ops), len(self.mid_gathers), len(self.host_gathers)):
        raise _RKGenericReject
      plan = _typed_load_gather(u, dtype, self.out_index, self.count, 0)
    ...
    if dtype is dtypes.float:
      ...
      plan = _typed_load_gather(u, dtype, self.out_index, self.count, fill_bits)
      ...
      for group,start in enumerate(groups):
        ...
        self.ew_ops.append(RKEWOp(replace(aligned.arg, addend=group*16), source, zero.arg, lanes, _EW_CFG[Ops.ADD]|_EW_STAGE_FP32_IN, stateful=True))
    ...
    if dtype is dtypes.bool:
      if runtime_address: raise _RKGenericReject
      if (typed_plan:=_typed_load_plan(u, dtype, self.out_index, self.count, require_offsets=True)) is None: raise _RKGenericReject
      plan = RKGather(param.arg.slot, 0, self.count, offsets=typed_plan.offsets,
    ...
    if runtime_address:
      ...
      if runtime_index is not None:
        ...
        try: index_lanes = _static_values(self.out_index, index_lane, self.count, int)
      ...
      if runtime_index is None:
        ...
        try:
          candidates = tuple(_static_values(self.out_index, index.substitute({runtime_load:runtime_load.const_like(candidate)}), self.count, int)
    ...
    return self._gather_slot(dtype, layout, replace(_typed_load_gather(u, dtype, self.out_index, self.count, fill_bits),
                                                   itemsize=itemsize), self.count*itemsize)
  ...
  def _pack_raw(self, u:UOp, components:Iterable[RKValue|RKArg], layout:RKLayout, mask:int|None=None,
    ...
    for byte,source in enumerate(parts): self._byte_gather(source.arg, result.arg, self.count, base=source.arg.addend, source_stride=2,
  ...
  def _coerce_bool(self, value:RKValue, layout:RKLayout) -> RKValue:
    ...
    self.ew_ops.append(RKEWOp(result.arg, value.arg, value.arg, self.count, _EW_CFG[Ops.MAX], submit_barrier=True, stateful=True, int16_output=True))
    return result
    ...
    if u.op is Ops.CMPNE:
      for expression,marker in (u.src, u.src[::-1]):
        if marker.op is Ops.CONST and marker.dtype.scalar() is dtypes.bool and bool(marker.arg) and expression.op is Ops.CMPLT:
          ...
          one_arg = self._i16_const(1)
          inverse = self._i16(one_arg, less.arg, _EW_CFG[Ops.SUB])
          either_nan = self._i16(nan[0], nan[1], _EW_CFG[Ops.MAX])
          numeric = self._i16(one_arg, either_nan, _EW_CFG[Ops.SUB])
          result = self._i16(inverse, numeric, _EW_CFG[Ops.MUL], self._dst(u, dtypes.bool, RKLayout.BOOL_INT16).arg)
  ...
  def _integer_bitwise(self, u:UOp) -> RKValue:
    ...
    if u.dtype.scalar() is dtypes.int16 and u.op is Ops.AND:
      for marker,source in (u.src, u.src[::-1]):
        ...
        sign = self._i16_clamp_one(self._i16(high.arg, self._i16_const(127), _EW_CFG[Ops.SUB]))
        sign_scale = self._i16(sign, const128.arg, _EW_CFG[Ops.MUL])
        if mask == 0x7fff:
          return self._pack_raw(u, (low, self._i16(high.arg, sign_scale, _EW_CFG[Ops.SUB])), RKLayout.INT16, mask)
    ...
    if u.dtype.scalar() is dtypes.int16 and u.op is Ops.OR:
      ...
      if all(mask is not None for mask in masks) and typing_cast(int, masks[0]) & typing_cast(int, masks[1]) == 0:
        ...
        summed_parts = tuple(self._i16(lhs.arg, rhs.arg, _EW_CFG[Ops.ADD]) for lhs,rhs in zip(*parts))
        return self._pack_raw(u, summed_parts, RKLayout.INT16, typing_cast(int, masks[0]) | typing_cast(int, masks[1]))
    if u.op is Ops.XOR:
      for marker, source in (u.src, u.src[::-1]):
    ...
    raw = tuple(self._byte_gather(value.arg, allocate(), lanes, base=value.arg.addend, dst_stride=2, itemsize=1,
                                  after=len(self.ew_ops)) for value in values)
    constants = {number:allocate() for number in (0, 1, 2, 3, 4, 7, 8, 15, 16, 31, 32, 63, 64, 127, 128)}
    self.gathers.extend(RKGather(self.out_param.arg.slot, constant.index, lanes, values=(number,)*lanes)
                        for number,constant in constants.items())
    ...
    for bit,(left,right) in enumerate(zip(lhs_bits, rhs_bits)):
      combined = allocate()
      if u.op is Ops.XOR:
        self.ew_ops.extend(_ew_ops(((combined, left, right, Ops.SUB), (combined, combined, combined, _EW_CFG_ABS)), lanes, **_INT16_EW))
      else:
        self.ew_ops.extend(_ew_ops(((combined, left, right, _EW_CFG_MIN if u.op is Ops.AND else Ops.MAX),), lanes, **_INT16_EW))
      if bit: self.ew_ops.extend(_ew_ops(((combined, combined, constants[1<<bit], Ops.MUL),), lanes, **_INT16_EW))
  ...
  def _int32_shift(self, u:UOp) -> RKValue:
    ...
    if shift_part is None:
      ...
      self._byte_gather(shift_part.arg, raw, self.count, base=shift_part.arg.addend//2,
    ...
    for absolute_bit in range(32):
      ...
      self._byte_gather(planes[plane], bits, self.count, base=planes[plane].addend//2+byte*vector_lanes,
                        dst_addend=absolute_bit*vector_lanes, after=len(self.ew_ops))
    for bit,mask in enumerate(masks):
      self._byte_gather(planes[bit], mask, matrix_lanes, base=planes[bit].addend//2+4*vector_lanes,
                        source_limit=vector_lanes, dst_addend=mask.addend//2, after=len(self.ew_ops))
    self._byte_gather(planes[7], sign, matrix_lanes, base=planes[7].addend//2+3*vector_lanes,
                      source_limit=vector_lanes, dst_addend=sign.addend//2, after=len(self.ew_ops))
  ...
  def _i16(self, lhs:RKArg, rhs:RKArg, cfg:int, dst:RKArg|None=None) -> RKArg:
    if dst is None: dst = self._scratch(dtypes.int16, RKLayout.INT16).arg
  ...
  def _i16_equal(self, lhs:RKArg, rhs:RKArg, diff:RKArg|None=None) -> RKArg:
    diff = self._i16(lhs, rhs, _EW_CFG[Ops.SUB]) if diff is None else diff
  ...
  def _i16_twos_complement(self, raw:tuple[RKArg, ...], sign:RKArg) -> tuple[RKArg, ...]:
    carry, result = sign, []
    for byte in raw:
      inverted = self._i16(self._i16(self._i16_const(255), self._i16(byte, byte, _EW_CFG[Ops.ADD]), _EW_CFG[Ops.SUB]), sign, _EW_CFG[Ops.MUL])
  ...
  def _int32_divmod(self, u:UOp) -> RKValue:
    ...
    if key not in self.int32_divmod:
      ...
      for bit_index in range(31, -1, -1):
        ...
        for byte in remainder:
          carry = self._i16_clamp_one(self._i16(byte, self._i16_const(127), _EW_CFG[Ops.SUB]))
        ...
        for left,right in zip(reversed(remainder), reversed(denominator)):
          ...
          byte_equal = self._i16_equal(left, right, diff)
    ...
    return self._pack_raw(u, self._i16_twos_complement(packed_raw, sign), RKLayout.INT32)
  ...
  def _raw_where(self, u:UOp) -> RKValue:
    ...
    if len(specials) == 1:
      special, finite_index = specials[0], 1-specials[0]
      finite = self.lower(u.src[1+finite_index])
    ...
    mask = self._coerce_bool(selector, RKLayout.BOOL_INT16)
    ...
    for yes_byte,no_byte in zip(yes_bytes, no_bytes):
      ...
      self._emit(selected, mask, delta, _EW_CFG[Ops.MUL])
  ...
  def _where(self, u:UOp) -> RKValue:
    ...
    if u.dtype.scalar() is dtypes.bool:
      ...
      values = [self._static(src, preferred) if value is None else self._coerce_bool(value, preferred) for src,value in zip(u.src, dynamic)]
  ...
  def _widen_int16(self, u:UOp, source:RKValue) -> RKValue:
    ...
    value = RKValue(self.out if u is self.root else self._scratch(dtypes.int, RKLayout.INT32).arg, dtypes.int, self.count, RKLayout.INT32)
  ...
  def _math(self, u:UOp) -> RKValue:
    ...
    if (recipe:=_DPU_MATH[u.op](u.src[0])) is None: raise _RKGenericReject
  ...
  def finish(self) -> RKImage:
    ...
    if (dtype is dtypes.half and result.layout in (RKLayout.FP16, RKLayout.BOOL_MASK, RKLayout.INT_FP16) or
      ...
      tiles = self._scratch(dtypes.int, RKLayout.INT32, _int32_tiles_bytes(self.count))
      self.ew_ops.append(RKEWOp(self.out, result.arg, tiles.arg, self.count, _EW_CFG[Ops.MAX], stateful=True, int32_output=True,
...
def _expand_math_uops(root:UOp, *, accurate_adds:bool=True) -> UOp:
  ...
  def physical_recipe(recipe:UOp, opaque:tuple[UOp, ...]=()) -> UOp:
    ...
    tagged_root = _tag_precise_adds(recipe.substitute(placeholders))
...
def _substitute_static_ranges(root:UOp, replacements:dict[UOp, UOp]) -> UOp:
  cache:dict[UOp, UOp] = {}
  for node in root.toposort():
    cache[node] = replacements[node] if node in replacements else node.replace(src=tuple(cache[src] for src in node.src))
  return cache[root]
...
def _unroll_static_local(uops:list[UOp], root:UOp) -> UOp:
  ...
  def expand_load(load:UOp, env:dict[UOp, int]) -> UOp:
    ...
    if len(stores) == 1 and stores[0].src[0].op is Ops.INDEX and load.src[0].op is Ops.INDEX:
      ...
      load_axes = _index_ranges(load_index)
      ...
      expanded = _substitute_static_ranges(expand_dependencies(stores[0].src[1], buffer, env), {axes[0]:load_index})
      ...
      return _substitute_static_ranges(expanded, {axis:axis.const_like(value) for axis,value in env.items()}) if env else expanded
...
def _lower_static_local_child(definition:_RKStaticLocalDef, slot:int, dtype:DType=dtypes.half, *, end:bool=False,
  ...
  fake = UOp.param(slot, dtype, (groups,)); store = fake.index(flat).store(definition.term).end(*definition.loops) if end else fake.index(flat).store(definition.term)  # noqa: E501
  child = _lower_uop_program(list(store.sink().toposort()), vectorize_reductions=False, recipes_ready=recipes_ready)
...
def _lower_multi_scalar_local_reductions(output:RKOutput, uops:list[UOp]) -> RKImage|None:
  ...
  local_loads = list(_semantic_loads(root, local=True))
  ...
  if any(_semantic_loads(definition.term, local=True) for definition in definitions.values()): return None
  next_slot = 1+max((u.arg.slot for u in uops if u.op is Ops.PARAM), default=out.arg.slot); staged:RKImage|None = None; sources:dict[UOp, UOp] = {}
  for buffer in buffers:
    fake_slot, next_slot = next_slot, next_slot+1; child = _lower_static_local_child(definitions[buffer], fake_slot, recipes_ready=True)
  ...
  substitutions = {load: sources[typing_cast(UOp, _local_buffer(load))].index(0).load().cast(load.dtype.scalar())
                   if load.dtype.scalar() is not dtypes.half else sources[typing_cast(UOp, _local_buffer(load))].index(0).load()
                   for load in local_loads}
  post = _lower_post_image(store, store.src[0].src[1], root, substitutions)
  if post is None or (appended:=_append_inplace_image(typing_cast(RKImage, staged), post)) is None: return None
  aliases = {fake.arg.slot:RKArg(RKBufferKind.SCRATCH, len(appended.scratch)+i) for i,fake in enumerate(sources.values())}; return replace(_alias_image_args(appended, aliases), scratch=appended.scratch+tuple(RKScratch(64) for _ in sources))  # noqa: E501
...
def _lower_vectorized_scalar_local_extrema(output:RKOutput, uops:list[UOp]) -> RKImage|None:
  ...
  value_buffers = [buffer for buffer,definition in definitions.items() if buffer.dtype.scalar() is dtypes.half and definition.update_op is Ops.MAX]
  index_buffers = [buffer for buffer,definition in definitions.items() if buffer.dtype.scalar() is dtypes.int and definition.update_op is Ops.MAX]
  ...
  if (value_def.initial.op is not Ops.CONST or float(value_def.initial.arg) != -math.inf or
      index_def.initial.op is not Ops.CONST or int(index_def.initial.arg) != dtypes.int.min or
      not value_def.loops or len(value_def.loops) != len(index_def.loops)): return None
  extents = tuple(int(loop.src[0].arg) for loop in value_def.loops if loop.src[0].op is Ops.CONST)
  if len(extents) != len(value_def.loops) or any(not 1 <= extent <= min(32767, _MAX_EW_ELEMS_FP16) for extent in extents) or \
     tuple(int(loop.src[0].arg) for loop in index_def.loops) != extents: return None
  ...
  casts = [term for term in weighted_terms if term.op is Ops.CAST and term.dtype.scalar() is dtypes.int and term.src[0].dtype.scalar() is dtypes.bool]
  ...
  current = next((src for src in predicate.src if (load:=_local_load(src)) is not None and _local_buffer(load) is value_buffer), None)
  ...
  if _strip_cast(_substitute_static_ranges(candidate, dict(zip(index_def.loops, value_def.loops)))).key != \
     _strip_cast(value_def.term).key: return None
  try: coordinates = tuple(_eval_int(coordinate, env) for env in _iter_range_env(list(index_def.loops), None, False))
  except RuntimeError: return None
  if len(coordinates) != total or any(not 0 <= value <= 32767 for value in coordinates): return None
  final_loads = [load for load in _semantic_loads(root, local=True) if _local_buffer(load) is index_buffer]
  ...
  child_data = _lower_static_local_child(value_def, 1+max((u.arg.slot for u in uops if u.op is Ops.PARAM and u.arg is not None), default=out_param.arg.slot), end=True, materialize=total)  # noqa: E501
  if child_data is None: return None
  ...
  lhs = UOp.param(fake_values, dtypes.half, (total,)).index(lane).load(); rhs = UOp.param(fake_best, dtypes.half, (total,)).index(lane).load()
  coordinate_values = UOp.param(fake_coordinates, dtypes.int16, (total,)).index(lane).load()
  equal = UOp(Ops.CMPEQ, dtypes.bool, src=(lhs, rhs)).cast(dtypes.int16)*coordinate_values
  selected = UOp.param(fake_out, dtypes.int16, (total,)).index(lane).store(equal).end(lane)
  selected_image = _lower_uop_program(list(selected.sink().toposort()), vectorize_reductions=False)
  ...
  scratch, gathers = list(combined.scratch), list(combined.gathers); ops, mid = list(combined.ew_ops), list(combined.mid_gathers); scratch.append(RKScratch(_scratch_bytes(total)))  # noqa: E501
...
def _lower_uop_program(uops:list[UOp], *, vectorize_reductions:bool=True, recipes_ready:bool=False) -> RKImage|None:
  ...
  if len(output_stores) > 1:
    ...
    for store in output_stores:
      child = _lower_uop_program(list(UOp(Ops.SINK, src=(store,)).toposort()), vectorize_reductions=vectorize_reductions, recipes_ready=recipes_ready)
      if child is None: return None
  ...
  if vectorize_reductions and \
     (multi_local:=_try(local_output, dtypes.half, _lower_multi_scalar_local_reductions, uops)) is not None: return multi_local
  ...
  if vectorize_reductions and \
     (local_extrema:=_try(local_output, dtypes.int, _lower_vectorized_scalar_local_extrema, uops)) is not None: return local_extrema
  ...
  if any(u.dtype.scalar() is dtypes.float for u in uops):
    if (sink:=next((u for u in uops if u.op is Ops.SINK), None)) is not None:
      ...
      if (storage_output:=_admit(local_output, dtypes.half)) is not None:
        ...
        root_storage = _typed_cast_source(storage_root, dtypes.half, dtypes.float) is not None
        storage_product_adds = any(_storage_product(boundary, storage_root) for boundary in storage_root.toposort())
      ...
      storage_uops = list(graph_rewrite(storage_sink, _pm_generic_storage_precision, name="rockchip generic storage precision").toposort())
  if vectorize_reductions and \
     (mul_add:=_try(strict_output, dtypes.half, _lower_vectorized_mul_add_reduction, uops)) is not None: return mul_add
  ...
  if vectorize_reductions and \
     (mapped_loop:=_try(local_output, dtypes.half, _lower_mapped_add_loop_reduction, uops)) is not None: return mapped_loop
  if vectorize_reductions and \
     (reduction:=_try(strict_output, dtypes.half, _lower_vectorized_unrolled_add_reduction, uops)) is not None: return reduction
  if (output:=_generic_output(local_output if storage_uops is None else
                              _output_store(uops:=storage_uops, accepted, allow_local=True))) is None:
  ...
  if output[1].dtype.scalar() is dtypes.int and (coordinates:=_coords(output)) is not None: return coordinates
  try:
    if not _contiguous_output(output[3], output[2]) and _static_values(output[3], output[3], output[2], int) != tuple(range(output[2])): return None
    ...
    if len(expanded_nodes) > _MAX_GENERIC_EXPANDED_NODES and os.getenv("ROCKCHIP_UOPS_DEBUG", "0") == "1": \
      raise _RKGenericReject(f"expanded nodes {len(expanded_nodes)}")
    ...
    image = RKContext(output, accurate_adds=_accurate(output, recipes_ready, storage_uops, storage_product_adds, expanded_nodes)).finish()
...
def _native_min(lhs:UOp, rhs:UOp) -> UOp: return UOp(Ops.MAX, lhs.dtype, src=(lhs, rhs), arg=_NATIVE_MIN)
...
def _mask_mul(lhs:UOp, rhs:UOp) -> UOp: return UOp(Ops.MUL, dtypes.half, src=(lhs, rhs), arg=_NATIVE_MASK_MUL)
...
_pm_storage_common = PatternMatcher([(UPat((Ops.ADD, Ops.MUL), dtypes.float, name="x"),
...
_pm_generic_storage_precision = PatternMatcher([(UPat(Ops.WHERE, dtypes.float, name="x"), lambda x:None if _is_static_expr(x) else
...
def _dpu_sqrt(source:UOp) -> UOp|None:
  ...
  result = source.alu(Ops.FDIV, estimate)
  return result.alu(Ops.ADD, invalid_factor.alu(Ops.SUB, one))

def _native_floor(source:UOp) -> UOp: return UOp(Ops.MAX, dtypes.half, src=(source, source), arg=_NATIVE_FLOOR)
...
def _dpu_periodic_reduce(source:UOp, reciprocal_period:float, split:tuple[float, ...], half_period:float) -> tuple[UOp, UOp, UOp]:
  ...
  one = UOp.const(1.0, dtypes.half)
  bounded = _native_min(source.cast(dtypes.half).alu(Ops.MAX, UOp.const(-10000.0, dtypes.half)), UOp.const(10000.0, dtypes.half))
...
def _dpu_sin(source:UOp) -> UOp:
  ...
  sign = one.alu(Ops.SUB, _positive_mask(UOp.const(0.0, dtypes.half).alu(Ops.SUB, reduced)).alu(Ops.MUL, UOp.const(2.0, dtypes.half)))
...
def _dpu_exp2(source:UOp) -> UOp:
  ...
  result = _poly_horner(fraction, (1, 0.6931471806, 0.2402265069, 0.0555041087, 0.0096181291, 0.0013333558)).alu(
...
def _dpu_log2(source:UOp) -> UOp:
  ...
  z2 = z.alu(Ops.MUL, z)
  result = exponent.alu(Ops.ADD, z.alu(Ops.MUL, _poly_horner(z2, (1, 1/3, 1/5, 1/7, 1/9))).alu(
    Ops.MUL, UOp.const(2/math.log(2), dtypes.half)))
...
def _fp16_fallback(uops:list[UOp]) -> list[UOp]:
  sink = graph_rewrite(next(u for u in uops if u.op is Ops.SINK), _pm_exp2_fallback, name="rockchip exp2 fallback")
  return list(graph_rewrite(sink, _pm_storage_common, name="rockchip fallback storage").toposort())
...
class RockchipRenderer(Renderer):
  ...
  def render(self, uops:list[UOp]) -> str:
    ...
    if image is None: image = _lower_uop_program(_fp16_fallback(uops), recipes_ready=True)
