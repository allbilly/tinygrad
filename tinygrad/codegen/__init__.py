...
def early_simplify(ast:UOp, optimize:bool=True) -> UOp:
  """Preserve fixed-function semantics while normalizing movement and affine range expressions."""
...
def do_to_program(ast:UOp, renderer:Renderer) -> UOp:
  ...
  if ast.op is Ops.PROGRAM: prg = ast
    ...
    if (native_prg:=renderer.native_program(early_sink)) is not None: prg = native_prg
    else:
