...
def hand_coded_optimizations(k:Scheduler) -> Scheduler:
  ...
  # Renderers with direct reduction ownership retain ranged REDUCE nodes for their own physical planner.
  try:
    if not (k.ren is not None and k.ren.direct_reduces) and k.unrollable_dims and \
       (k.upcast_size() <= 4 or not k.axes_of(AxisType.UNROLL)) and k.upcast_size() < 64:
