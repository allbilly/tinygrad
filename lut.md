...
ASINH uses `ASINH_CORE = 64` (SHA256 `b3c6e1ac213e3a2d2a761bdfef8307deaf0f22ffdee8750b6a53988d6853ff99`) and `ASINH_RANGE = 65` (SHA256 `6bbc9710217283be5d345379952b70dd5043bdf1b3dc966c5f1ac729f5781fbb`). The core table's negative bank amplifies the center while its positive bank covers magnitudes through two. The range table's negative bank covers `[2,18]`; its positive bank receives `x/19` for large inputs. Device masks reconstruct four regions and sign. The range table's shared LE zero knot is corrected offline to the middle-region value. The 46-stage, eight-scratch program passes the official method and strict 4,097-point `[-32,32]` hardware sweep.

ACOSH is deliberately distinct because its singular `x=1` endpoint needs residual coordinates instead of ASINH sign reconstruction. `ACOSH_CORE = 66` (SHA256 `0f86849647a3df1f518d536f83b64e5cadb04a33c0b029e2304cf283a2091ebb`) uses its negative bank for amplified `acosh(1+d)` near `d=x-1=0` and its positive bank for the remainder through `x=2`. `ACOSH_RANGE = 67` (SHA256 `921a1f64176dbfcbf4fbc6f658ffc78ca25d158bee3eb36495feefd4e232e533`) shares its negative bank with the middle interval and receives `x/19` in its positive bank for large inputs. The generator corrects the shared LE zero knot to the middle-region value. Device masks select local, broad, middle, and large regions; multiply by a native `valid/valid` expression to synthesize NaN below one; and explicitly zero the exact endpoint. The typed plan has 43 stages, eight reusable scratch buffers, and one instance of each physical LUT task. It passes the official method and strict 4,097-point `[1,32]` hardware sweep plus `[-1,0,1,nan]` without tolerance relaxation or host semantic execution.
