...
def test_cmac_asset_hash_is_revalidated_before_device_effects() -> None:
  program, effects = _setup(_cmac_asset_native())
  asset = effects.native.assets[0]
  object.__setattr__(asset, "payload", b"x" * asset.size)
  with pytest.raises(PhysicalRuntimeReject, match="asset"):
    effects._preflight()
  assert program.dev.events == []
...
def test_native_plan_identity_is_revalidated_before_patch() -> None:
  program, effects = _setup(_cmac_native())
  effects._preflight()
  effects.native = replace(effects.native, flags=1)
  with pytest.raises(PhysicalRuntimeReject, match="native_schema"):
    effects.execute(preflight=False)
  assert effects.closed and not effects.ownership_unknown
  assert all(event[0] not in ("reset", "submit") for event in program.dev.events)
...
def test_unproven_reset_is_fail_closed_and_has_zero_effects() -> None:
  program, effects = _setup(_cmac_native(), reset_live_proven=False)
...
def test_driver_contract_constants_are_action_six_and_one_submit() -> None:
  ...
  assert rkp.CMAC_V1_RESET == (6, 0) and rkp.CMAC_V1_SUBMIT == (0x5, 6000, 1, -1, 1)
