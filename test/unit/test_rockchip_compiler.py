...
class TestDPUCompiler(unittest.TestCase):
  ...
  def test_wide_affine_max_atoms_reduce_scalars_then_gather(self):
    ...
    for dilation in ((2,3),(3,2),2,3):
      plan = lower_affine_max_result(sink(Tensor.empty(3,2,17,14,dtype=dtypes.half).max_pool2d((5,5),dilation=dilation))).plan
      self.assertIsInstance(plan, RKProgram)
      assert isinstance(plan, RKProgram)
      image = emit_program(plan)
      self.assertLessEqual(len(image.stages), 400)
      ...
      self.assertFalse(contains_uop(plan))
      plans.append(plan)
    self.assertTrue(any(len(plan.scratch) == 4 for plan in plans))
  ...
  def test_bool_fill_copy_and_or_use_int8_dpu(self):
    ...
    expressions = (Tensor.full((65,),True,dtype=dtypes.bool),lhs.maximum(False),lhs.maximum(rhs),lhs.minimum(rhs),lhs.logical_not())
    for expression in expressions:
      result = lower_native(sink(expression))
      self.assertIs(result.kind,RKLowerKind.NATIVE)
      self.assertIsInstance(result.plan,RKDPUProgram)
    ...
    and_stage = cast(RKDPUProgram,lower_native(sink(lhs.minimum(rhs))).plan).stages[0]
    self.assertIsInstance(and_stage,RKALUStage)
    self.assertIs(cast(RKALUStage,and_stage).op,Ops.MUL)
  ...
  def test_already_packed_broad_gemm_legalizes_to_one_cmac_task(self):
    for m,n,k in ((16,16,16),(32,32,32),(64,64,32),(16,16,64),(1,128,128)):
  ...
  def test_masked_affine_product_materializes_multiplicative_identity(self):
    result = lower_native(sink(Tensor.empty(10,dtype=dtypes.half).cumprod(0)))
    self.assertIs(result.kind, RKLowerKind.NATIVE)
    self.assertIsInstance(result.plan, RKProgram)
    assert isinstance(result.plan, RKProgram)
    image = emit_program(result.plan)
