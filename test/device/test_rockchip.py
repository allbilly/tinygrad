import ctypes, glob, math, os, unittest
import numpy as np
from tinygrad import Tensor, dtypes
from tinygrad.device import Target, TinyELF
from tinygrad.renderer.rockchip import (RKArg, RKBufferKind, RKDPUProgram, RKStridedAtomGatherStage, emit_dpu, encode_image)
...
class TestRockchip(unittest.TestCase):
  ...
  def test_int32_fill_uses_exact_dpu_output_offset(self):
  ...
  def test_int32_or_all_ones_uses_exact_fill(self):
    values = np.resize(np.array([dtypes.int.min,-1234,-1,0,1,1234,dtypes.int.max],dtype=np.int32),65)
    actual = (Tensor(values,device="ROCKCHIP",dtype=dtypes.int) | 0xffffffff).realize().numpy()
    np.testing.assert_equal(actual,np.full(65,-1,dtype=np.int32))
