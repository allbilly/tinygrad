...
class NativeRenderer(Renderer):
  def native_program(self, ast:UOp) -> UOp|None:
    return UOp(Ops.PROGRAM, src=(ast, UOp(Ops.BINARY, arg=b"native")), arg=ProgramInfo.from_sink(ast, self.target))
...
class TestNativeProgram(unittest.TestCase):
  def test_renderer_can_intercept_early_sink(self):
    ...
    self.assertEqual(program.src[1].op, Ops.BINARY)
    self.assertEqual(program.src[1].arg, b"native")
